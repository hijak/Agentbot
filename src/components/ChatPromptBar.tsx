...
const DICTATION_ENDPOINT_MS = 1500;
...
function dictateOnce(
  ...
  if (bridge?.speechStart && bridge.onSpeechTranscript && bridge.onSpeechEnd) {
    return new Promise((resolve, reject) => {
      ...
      void bridge.speechStart({ endpointMs: DICTATION_ENDPOINT_MS }).catch((err) =>
...
function dictationErrorForReason(reason?: string): string {
  switch (reason) {
    case "speech-not-authorized":
      return "macOS denied Speech Recognition for the Agentbot speech helper. Enable Agentbot or Agentbot Speech in System Settings → Privacy & Security → Speech Recognition, then restart Agentbot.";
    case "mic-failed":
      return "macOS could not open the microphone for dictation. Enable Microphone access for Agentbot or Agentbot Speech in System Settings → Privacy & Security → Microphone.";
    ...
    case "recognition-error":
      return "macOS rejected the audio stream. Check Microphone and Speech Recognition access for Agentbot or Agentbot Speech.";
    default:
      return reason
...
export const filesToHostedAttachments = filesToAttachments;
...
export function ChatPromptBar({
  ...
  const handleDictate = useCallback((onTranscript?: (text: string) => void) => {
...
export const HostedPromptBar = ChatPromptBar;
