...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "agentbot.backup") {
  ...
  if (root.format === "agentbot.package") return packagePreview(root, manifest);
  if (root.format !== "agentbot.team") throw new Error("This is not an Agentbot backup, BotMRR playbook or legacy team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "agentbot.package", version: 1, package: pkg };
}
