...
const SEND = "node --experimental-strip-types scripts/control-agentbot.ts send --bot x --text y";
...
describe("parseControlCommand", () => {
  it.each([
    ...
    [SEND, "send"],
    [PRESS, "press"],
    ["cd repo && node scripts/control-agentbot.ts doctor", "doctor"],
    ["node scripts\\control-agentbot.ts doctor", "doctor"],
    ["FOO=1 pnpm control:omb wait --bot b", "wait"],
    ["timeout 30 npx tsx ./scripts/control-agentbot.ts screenshot > out.png 2>&1", "screenshot"],
  ...
  it.each([
    "cat scripts/control-agentbot.ts",
    "sed -n 1,40p scripts/control-agentbot.ts",
    "git log -- scripts/control-agentbot.ts",
    "grep -n doctor scripts/control-agentbot.ts",
    "cat docs/access-control-agentbot.tsx",
...
describe("runSteps", () => {
  it("lists every command from every driver shape, in order, reads left out, control-CLI steps verified", () => {
    const steps = runSteps([
      ...
      claude("cat scripts/control-agentbot.ts", true),
      claude(PUSH, true),
    ]);
