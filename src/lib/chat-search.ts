...
/** Return the strongest matching transcript line as a short sidebar preview. */
export function fuzzyMessageMatch(
  messages: readonly { content: string }[],
  query: string,
): MessageSearchMatch | null {
  const match = fuzzySearch(messages, query, (message) => message.content)[0];
  if (!match) return null;
  const compact = match.content.replace(/\s+/g, " ").trim();
  return { snippet: compact.length > 100 ? `${compact.slice(0, 100).trimEnd()}…` : compact };
}
