...
folder on the computer running Agentbot — nothing is stored anywhere else,
...
disk, those buttons only work from the computer running Agentbot — a
...
`~/.agentbot/memory-journal/<botId>.ndjson`. It lives *outside* the
