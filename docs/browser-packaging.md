...
The server receives `AGENTBOT_RESOURCES_PATH` from Electron. It resolves an explicit
`AGENTBOT_AGENT_BROWSER_PATH` override first, then the complete bundled engine and
...
AppArmor policy for the root-owned browser executable under `/opt/Agentbot`.
...
On macOS, use `/absolute/Agentbot.app/Contents/Resources`. On Linux, run as
an unprivileged user against the installed `.deb` at
`/opt/Agentbot/resources`. This check creates its own empty home and local
...
Windows uses an explicitly identified Agentbot build of agent-browser
...
native-verified `0.36.0-agentbot.1` revision and does not yet get the new recording
