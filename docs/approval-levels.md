...
provider's own permission modes, passed through. Agentbot does not judge an
...
| **Custom (`config.toml`)** | Codex only. Agentbot reads and reapplies the effective approval and sandbox settings from your Codex configuration. |
...
ends. Agentbot keeps no standing grant for a provider's tool. It is not
...
Native modes require a CLI version that supports them; Agentbot does not
silently substitute unrestricted access when a mode is rejected.

### Read-only integration tools

Agentbot's built-in agents MCP server describes nine scoped reads with
...
provider's session. Its quieter default is Full access; Agentbot keeps its
