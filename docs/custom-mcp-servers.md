...
Agentbot saves a new server switched off. Use **Test** to start the command
...
`AGENTBOT_CLAUDE_INHERIT_USER_CONFIG=1` on the Agentbot process remains the full
...
window with `--autocompact` (2.1.122+). Agentbot reads `claude --version`
...
The same registry lives in `~/.agentbot/config.json`:
...
by hand, restart Agentbot. Every bot whose engine can mount custom MCP
...
- **Testing is bounded.** A command is stopped after the handshake (or eight
  ...
  inherits none of Agentbot's workspace or provider credentials; only the
...
- **The result gate covers commands.** Oversized tool results from a stdio
  server are trimmed before they reach the model (`AGENTBOT_MCP_RESULT_BUDGET`).
