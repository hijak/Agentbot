# Deploy Agentbot on a VPS

From a blank Linux server to Agentbot running on it around the clock, reachable from your laptop, the desktop app and your phone, with your bots working while every laptop is closed. It assumes nothing beyond being able to open a terminal and paste commands. About twenty minutes, most of it waiting.

Two ways to make the server reachable are covered. Pick one; the rest of the guide is the same.

| | You need | Who can reach it | Best for |
|---|---|---|---|
| **A. Your own domain** (Docker + Caddy) | a domain name, ports 80/443 | anyone with a pairing code, over HTTPS | a permanent address you own |
| **B. Your Tailscale network** (`serve --tailscale`) | Tailscale on the server and your devices | only your tailnet | the most private; nothing public at all |

Whichever you pick, the login is the same: you **pair** each device once with a short code and it stays signed in. A session lasts 30 days; using it with half that or less left renews it to a full 30, up to 180 days from pairing (`AGENTBOT_SESSION_TTL_DAYS` and `AGENTBOT_SESSION_MAX_DAYS` change both numbers). There is no password.

## Hetzner launch check (2026-09-07)

An earlier managed-tunnel path was exercised on a blank
Hetzner CX43 (x86-64, Ubuntu 24.04), using Node 24.20.0 and Codex 0.153.4,
before that path was removed. New installs should use Path A (own domain)
or Path B (Tailscale) below. The historical run is preserved in
[hetzner-launch-2026-09-07](verification/hetzner-launch-2026-09-07.md).
...
2. **Node 24 or newer** for path B (`node --version`). On Ubuntu:

   ```sh
   curl -fsSL https://deb.nodesource.com/setup_24.x | sudo -E bash - && sudo apt-get install -y nodejs
   ```

    Path A uses Docker instead and needs no Node on the host.
...
For path B, use the same unprivileged Linux account for setup and the
...
## Path A: your own domain, with Docker
...
3. **Get the deploy files and set the name:**
   ...
   ```sh
   git clone https://github.com/milind-soni/Agentbot && cd Agentbot/deploy
   cp .env.example .env
...
In this path, every `npx agentbot …` command below is run inside the container instead:

```sh
docker compose exec omb node dist-server/agentbot.js pair --label "My MacBook"
```

## Path B: only your Tailscale network
...
Bots can browse on a server. Docker (path A) ships the engine and Chrome in
the image. For path B, install Linux system libraries once from the
administrator shell:

```sh
sudo -H npx --yes agentbot browser install --with-deps
```

Then install the browser in the service account's home, from the `maus` shell:

```sh
npx --yes agentbot browser install
```
...
The npm Agentbot package does not install model engine CLIs. For path B,
...
engines you use. For path A, run the installed CLI inside the container, for
example `docker compose exec omb claude`.

Engine logins belong to the service user's home (for example `~/.codex` and
`~/.claude`), separately from Agentbot's `~/.agentbot`. Keep that home when
...
```sh
npx agentbot pair --label "Kitchen iPad"
```

```
pairing code:  RR8Y-BLR6-H939
expires:       10:59:45 AM (single use)
open or scan:  https://maus.example.com/pair#code=RR8Y-BLR6-H939
```
...
```sh
npx agentbot sessions              # id, device, scope, last seen, expires
npx agentbot sessions revoke ID    # signs that device out and closes its stream at once
npx agentbot status                # what the server says about itself
```

## Keep it running

`npx agentbot serve` is a plain foreground process. For systemd (path B),
install a chosen release first, from the `maus` shell. Replace `X.Y.Z` with the
published version you want to run:

```sh
npm install --global --prefix "$HOME/.local" agentbot@X.Y.Z
```

Stop the foreground server with Ctrl-C before enabling the service. From the
administrator shell, save this as `/etc/systemd/system/agentbot.service`:

```ini
# /etc/systemd/system/agentbot.service
[Unit]
Description=Agentbot server
...
ExecStart=/home/maus/.local/bin/agentbot serve --tailscale --no-pair
Restart=always
RestartSec=5
...
```sh
sudo systemctl daemon-reload && sudo systemctl enable --now agentbot
journalctl -u agentbot -f            # the server's log
```

`--no-pair` skips printing a code at every restart; mint one with `npx agentbot pair` when you need it. Docker (path A) restarts on its own (`restart: unless-stopped`).
...
- **Path B with the service above:** choose the new published version,
  then run these commands from the administrator shell, replacing `X.Y.Z`:

  ```sh
  sudo systemctl stop agentbot
  sudo -iu maus npm install --global --prefix /home/maus/.local agentbot@X.Y.Z
  ...
  does not update the installed package. For foreground `npx` usage, specify the
   desired release as `npx --yes agentbot@X.Y.Z serve --tailscale`.
- **Path A:** `cd Agentbot/deploy && docker compose pull omb && docker compose up -d`.
...
process, or `sudo systemctl stop agentbot` from the administrator shell for
the service above. Stop any engine processes and managed desktops still writing
files you intend to back up.

For path B, this backs up application state (bots, chats, routines,
and paired sessions). Run it from the service account's shell:

```sh
umask 077
backup_dir=$(mktemp -d "$PWD/agentbot-backup.XXXXXX")
tar czf "$backup_dir/agentbot-data.tgz" -C "$HOME" .agentbot
```
...
For path A, the whole `/data` volume includes the container's CLI homes. From
`Agentbot/deploy`, stop the app before archiving; `deploy_data` is the default
volume name, so use your actual volume name if you changed the Compose project:

```sh
docker compose stop omb
backup_dir=$(mktemp -d "$PWD/agentbot-backup.XXXXXX")
docker run --rm -v deploy_data:/data:ro -v "$backup_dir":/b alpine sh -c 'umask 077; tar czf /b/agentbot-data.tgz -C /data .'
```
...
service with `sudo systemctl start agentbot`, or `docker compose start omb`.

## The rules the setup relies on

Read this before putting anything else in front of the server.

- The server only ever listens on loopback (`127.0.0.1:8799`). Never publish that port yourself. Caddy and Tailscale each reach it from the same machine.
- A request that arrives through a proxy is treated as remote and needs a session, whatever headers it carries. A proxy of your own (nginx, Traefik) must forward the real `Host` and add `X-Forwarded-For` and `X-Forwarded-Proto`, must not buffer the event stream, and must **not** rewrite `Host` to `127.0.0.1`.
- Pairing is the login. Want a second wall in front of it? Path A's `Caddyfile` has a commented `basic_auth` block for a shared password.
- The session cookie is marked `Secure`; do not serve this over plain HTTP on the public internet.
- The one thing a stranger can read is `/.well-known/agentbot/environment` (the server's id, label, version, capabilities) and `/api/health` (only the app name). Everything else answers "pair this device".

## Troubleshooting

**Path A: the page never loads or shows a certificate error.** Caddy could not get a certificate. Check that the name resolves to the server and that ports 80 and 443 are open; `docker compose logs caddy` shows the reason.
...
**What does the server think it is?** `https://<address>/.well-known/agentbot/environment` is public and shows its id, label, version and capabilities; `npx agentbot status` prints the same on the server.

**Something else.** `journalctl -u agentbot --since -10m` (or `docker compose logs omb --tail 100`) shows the server's startup lines. Paste them with your question in the community channel.
...
For path B, an administrator can allow only a trusted, root-owned Chrome
...
```sh
sudo install -d -o root -g root -m 0755 /opt/agentbot-browser
sudo cp -R /home/maus/.agent-browser/browsers/chrome-VERSION /opt/agentbot-browser/
sudo chown -R root:root /opt/agentbot-browser/chrome-VERSION
sudo chmod -R go-w /opt/agentbot-browser/chrome-VERSION
```

Save this as the root-owned `/etc/apparmor.d/agentbot-chrome`, replacing
...
profile agentbot-chrome /opt/agentbot-browser/chrome-VERSION/chrome flags=(unconfined) {
  userns,
}
```
...
```sh
sudo apparmor_parser -r /etc/apparmor.d/agentbot-chrome
```
...
```ini
Environment=AGENT_BROWSER_EXECUTABLE_PATH=/opt/agentbot-browser/chrome-VERSION/chrome
```
