...
On a real server, after `agentbot fleet init --domain <domain> --operator
<user>` as root: `systemctl status agentbot-fleet`, then `ls -l
/run/agentbot/fleet.sock` must show `root:<user>` and mode 660. Sign in to
the operator workspace and open Settings → Workspaces. Create one, add a
member, suspend and resume it, and confirm `/var/log/agentbot/fleet.jsonl`
