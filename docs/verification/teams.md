...
AGENTBOT_UI_E2E=1 pnpm exec vitest run scripts/testing/team-lifecycle-ui.e2e.test.ts scripts/testing/team-template-ui.e2e.test.ts --maxWorkers=1
AGENTBOT_UI_E2E=1 pnpm exec vitest run scripts/testing/team-canvas-ui.e2e.test.ts --maxWorkers=1
AGENTBOT_UI_E2E=1 pnpm exec vitest run scripts/testing/team-computers-ui.e2e.test.ts --maxWorkers=1
...
explicit empty-team deletion. API requests and `control-agentbot` wait/messages
...
The renderer test uses `control-agentbot ui launch`, opens **Create team**, leaves it
