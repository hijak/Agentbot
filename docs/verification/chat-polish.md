...
pnpm exec vitest run server/tool-summary.test.ts server/control-agentbot.test.ts server/drivers/claude.test.ts server/drivers/codex.test.ts server/drivers/pi.test.ts server/drivers/acp/acp.test.ts
pnpm typecheck
pnpm lint
pnpm check:contrast
```
...
`server-1789307799660-26000.log` in the temporary `agentbot-verification-evidence`
