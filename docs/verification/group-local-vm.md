...
tracked separately in [#860](https://github.com/milind-soni/Agentbot/issues/860).
...
`AGENTBOT_VERIFY_PODMAN` to the absolute executable path and `AGENTBOT_VERIFY_MACHINE` to that connection,
then run:
...
```sh
node node_modules/vitest/vitest.mjs run server/group-local-vm.e2e.test.ts server/vps-routing.test.ts server/local-vm-lease.test.ts server/group-goal-run.test.ts server/group-goal-run.e2e.test.ts server/group-goal-wait-cap.e2e.test.ts server/control-agentbot.test.ts
```
