...
The `ui` group of `control-agentbot` drives the real React renderer — the same
...
`scripts/testing/control-agentbot-ui.ts`). Everything it touches is disposable: the
...
8799 and `~/.agentbot` are never involved.

## Launch

```sh
node --experimental-strip-types scripts/control-agentbot.ts ui launch \
...
them. `AGENTBOT_AGENT_BROWSER_PATH` and `AGENT_BROWSER_EXECUTABLE_PATH` take
...
{
  "ok": true,
  "ui": "/tmp/agentbot-verify-data-XXXXXX/ui.json",
...
```sh
H=/tmp/agentbot-verify-data-XXXXXX/ui.json
...
The permanent form of this recipe is `scripts/testing/control-agentbot-ui.e2e.test.ts`:
...
reason otherwise; `AGENTBOT_UI_E2E=1` forces the verified download. The `ui-smoke`
...
```sh
AGENTBOT_UI_E2E=1 pnpm exec vitest run scripts/testing/stream-buffer.e2e.test.ts
...
optional `AGENTBOT_UI_E2E=1` gate. It verifies profile-only role creation, closing
