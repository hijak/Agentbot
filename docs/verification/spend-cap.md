...
granting `budgets` and `billing`) and launches the `control-agentbot` fixture with
...
For the same by hand, launch a fixture with `AGENTBOT_ENTERPRISE_DIR` pointing at a
folder whose `server/index.js` exports such a `register()`, and
`AGENTBOT_LICENSE_KEY` set to any value, then use the normal chat-turn commands.
