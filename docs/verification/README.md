...
Agentbot has one development control surface: `pnpm control:omb`. It is a
...
that exact child. The parent shell and the user's Agentbot data are
...
`control-agentbot ui` ([Chat UI, driven headlessly](chat-ui.md)) drives the real
...
service. It does not drive the People settings UI through `control-agentbot`.
