...
The optional functional test launches the real Agentbot server, its OpenCode
...
```sh
AGENTBOT_OPENCODE_E2E_CLI=/absolute/path/to/opencode pnpm exec vitest run server/opencode-variants.e2e.test.ts
```

Without `AGENTBOT_OPENCODE_E2E_CLI`, Vitest reports this test as skipped. A skip does
...
The checks require Agentbot to retain this catalog metadata, advertise its
...
Agentbot server with the same disposable data directory.
...
provider, an upstream gateway's routing, or the Agentbot GUI. The fixture
