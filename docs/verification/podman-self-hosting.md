...
`/mnt/c/Projects/Agentbot`:

```sh
cd "$repo"
podman build -f deploy/podman/Containerfile -t localhost/agentbot-podman:verify .
fixture=$(mktemp -d /tmp/omb-podman-verify-XXXXXXXX)
mkdir -m 700 "$fixture/.agentbot"
...
cat > "$fixture/.agentbot/config.json" <<EOF
...
AGENTBOT_IMAGE_TAG=verify
AGENTBOT_DATA_ROOT=$fixture
PODMAN_SOCKET=/run/user/$(id -u)/podman/podman.sock
AGENTBOT_PORT=28799
AGENTBOT_WEBHOOK_PORT=28800
AGENTBOT_HTTP_PORT=28880
AGENTBOT_PUBLIC_URL=http://localhost:28880
AGENTBOT_HTTPS_HOST=https-disabled.invalid
ENGINES=
EOF
...
Use the candidate checkout's `control-agentbot.ts` on the host. The explicit fixture
...
2. Use `control-agentbot.ts new-bot --name "Fixture A" --url URL` and repeat for B.
