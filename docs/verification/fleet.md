...
privileges, read-only system, the `${AGENTBOT_PORT}`-style expansion), the
...
```sh
agentbot fleet init --domain example.test --dry-run
agentbot fleet create acme --admin owner@acme.test --dry-run
```
...
1. Fresh Ubuntu 24.04 or newer, Caddy from apt, `npm install -g agentbot`,
   Claude Code installed once as root, a wildcard DNS record at the server.
2. `agentbot fleet init --domain <domain>`; check `systemctl status
   agentbot-fence` and `nft list table inet agentbot`.
3. `agentbot fleet create alpha --admin you@example.test --cap 1` and the
...
4. Isolation: as `omb-alpha` (`runuser -u omb-alpha -- curl -s
   ...
   as root it must answer. `ls /var/lib/agentbot/beta` as `omb-alpha` must
   be denied.
