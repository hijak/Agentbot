# Agentbot harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what Agentbot lacked by
...
**The reframe that reorders everything.** Agentbot is itself a harness. The
...
**On-disk forward compatibility.** `~/.agentbot` is shared across app versions.
