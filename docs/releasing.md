...
[Agentbot releases](https://github.com/milind-soni/Agentbot/releases).
...
0.1.46 point to `milind-soni/agentbot-releases`; newer builds point to
`milind-soni/Agentbot`. For that reason:

1. Every new release is published byte-for-byte to both repositories during
   the bridge period.
2. `agentbot-releases` must stay public. Do not delete its final bridge
...
Set these in **Agentbot → Settings → Secrets and variables → Actions**.
...
Fine-grained tokens** → repository access: only `agentbot-releases` →
