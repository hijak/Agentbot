...
Agentbot has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded Agentbot server on GNOME Xorg and GNOME Wayland.
...
progress in [issue #29](https://github.com/milind-soni/Agentbot/issues/29) and the safety hold in
[issue #345](https://github.com/milind-soni/Agentbot/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/milind-soni/Agentbot/issues/113). Xorg is tracked in
[issue #79](https://github.com/milind-soni/Agentbot/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/milind-soni/Agentbot/issues/109).

## Download packages

Choose one Ubuntu 24.04 x86_64 package from the latest release:

- [Debian package (`Agentbot-amd64.deb`)](https://github.com/milind-soni/Agentbot/releases/latest/download/Agentbot-amd64.deb) — recommended; APT installs its desktop dependencies and configures the bundled bot browser's sandbox.
- [Portable AppImage (`Agentbot.AppImage`)](https://github.com/milind-soni/Agentbot/releases/latest/download/Agentbot.AppImage) — does not install system files.
- [SHA-256 checksums](https://github.com/milind-soni/Agentbot/releases/latest/download/SHA256SUMS-ubuntu-x64.txt)

Versioned packages and previous releases remain available on the
[releases page](https://github.com/milind-soni/Agentbot/releases).
...
```sh
git clone https://github.com/milind-soni/Agentbot.git
cd Agentbot
corepack enable
...
- `release/Agentbot-<version>-amd64.deb`
- `release/Agentbot-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./Agentbot-amd64.deb
```

Then open **Agentbot** from the GNOME application launcher. To remove it:

```sh
sudo apt remove agentbot
```

The portable AppImage does not install system files:

```sh
chmod +x release/Agentbot-*-x86_64.AppImage
./release/Agentbot-*-x86_64.AppImage
```

For a downloaded release AppImage, use `Agentbot.AppImage` in place of the versioned path above.
...
Application data remains local in `~/.agentbot`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/agentbot` unless the environment overrides it).
...
./release/linux-unpacked/agentbot
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Agentbot
...
```sh
AGENTBOT_EXTRA_PATH=/your/custom/bin ./release/Agentbot-*-x86_64.AppImage
```

Restart Agentbot after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. Agentbot detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. Agentbot does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. Agentbot also
...
The Xorg runtime uses private sockets, standard permission mode, per-action Agentbot approvals,
...
then runs the same smoke against `/opt/Agentbot/agentbot`. These lanes prove the embedded server and UI are
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Agentbot. If it lives outside the
common directories above, use `AGENTBOT_EXTRA_PATH` while testing and report the install location so it can be
...
```sh
chmod +x Agentbot-*-x86_64.AppImage
file Agentbot-*-x86_64.AppImage
```
