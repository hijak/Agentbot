# Self-hosting the Agentbot server
...
The npm CLI supports Tailscale, your own domain, or your own proxy.
...
> a correctly configured proxy require a paired session.

Step by step, for a server you do not have yet: [Deploy Agentbot on a
VPS](deploy-vps.md) walks through the two ways in (own
...
- a browser for bots, once the engine is installed on the server
  (`npx agentbot browser install`, or nothing to do in the Docker image,
...
It then starts the server and keeps your data in `~/.agentbot`. If you
...
and creates no pairing invitation. Use `npx agentbot setup` to configure without starting, or
`npx agentbot serve` to start non-interactively with your existing config
...
directories, not all under `.agentbot`.
...
`npx agentbot browser install` as the service user so that user's browser
is present. Two ways to make the server reachable from elsewhere:

- **On your Tailscale network, no domain needed:**
  `npx agentbot serve --tailscale`. Tailscale terminates HTTPS with its
...
- **Your own domain, still one command:**
  ...
  ```sh
  npx agentbot serve --domain maus.example.com
  ```
...
- **Behind your own proxy or domain:** `npx agentbot serve --public-url
  https://maus.example.com`, with the proxy rules from "Putting a proxy in
  front".

Later: `npx agentbot pair --label "Kitchen iPad"` for another device
(`--client` for one that may chat but not change settings), and
`npx agentbot sessions` to see or revoke them. `agentbot serve` is a
...
The IP comes from this server's network interfaces, never the browser
or an IP-echo service. Only a single unambiguous public IPv4 is shown.
For containers/NAT or hosts with multiple public addresses, an administrator can
set `AGENTBOT_PUBLIC_IPV4` to the public IPv4 of the HTTPS proxy and restart OMB.
...
`AGENTBOT_WEBHOOK_PUBLIC_URL` or existing webhook URLs.
This feature is for self-hosted servers.
...
(`ghcr.io/milind-soni/agentbot`, tagged `latest`, `sha-…` and `v…`).
...
docker compose exec omb node dist-server/agentbot.js pair # prints a code, a link and a QR
```
...
AGENTBOT_DATA_DIR="$HOME/.agentbot" AGENTBOT_PORT=8799 \
...
```sh
npx agentbot service install --domain maus.example.com   # or --tailscale, or nothing
```

It renders a systemd unit (Linux) or a launchd agent (macOS) that runs the
same `agentbot serve …` with your options, restarts it if it stops, and,
...
`agentbot service uninstall` prints the reverse. Install the package
permanently first (`npm install -g agentbot`): a service must not point
...
- **Anthropic API key**: while one is saved, every Claude bot runs on it and
  ...
  page, `config.json`, or `AGENTBOT_ANTHROPIC_API_KEY`.
...
`agentbot fleet` runs one workspace per client on a single Linux server,
each as its own OS user, its own `agentbot@<name>` service on its own
...
```sh
agentbot fleet init --domain example.com
```

That writes the template unit, the fence and its unit, the workspace folders,
and adds `import /etc/caddy/agentbot.d/*.caddy` to `/etc/caddy/Caddyfile`. Then per
client:

```sh
agentbot fleet create acme --admin owner@acme.test --member @acme.test \
  --brand /root/acme-brand.json --anthropic-key-file /root/acme-anthropic.key \
  --cap 50 --memory 1G
agentbot fleet users acme add bob@acme.test --chat-only
agentbot fleet list
agentbot fleet suspend acme      # 503 page, service stopped; resume undoes it
agentbot fleet upgrade           # new release, then every running workspace restarted in turn
agentbot fleet delete acme --yes # add --keep-data to keep the home folder
```
...
agent's audit log at `/var/log/agentbot/fleet.jsonl`.

`https://acme.example.com` is up when `create` returns; the first admin signs
in with an emailed code. `AGENTBOT_LICENSE_KEY` in the environment (or
...
```sh
npx agentbot pair                         # npm install
pnpm omb pair                                # from a checkout
docker compose exec omb node dist-server/agentbot.js pair   # Docker
```

It prints a 12-character code (single use, five minutes) and, when the
server knows its public address (`AGENTBOT_PUBLIC_URL`, set by the Docker stack),
...
HTTPS address or full pairing link, with an optional name. Custom domains
work; Tailscale is not required. Generate a fresh
link for each device: `npx agentbot pair --label "My desktop"` creates an
...
`GET /.well-known/agentbot/environment` is public and tells a client what
...
```sh
AGENTBOT_SIGNIN_EMAILS="her@yourcompany.com, @yourcompany.com"   # full access
AGENTBOT_SIGNIN_MEMBER_EMAILS="freelancer@example.com"          # chat and approvals only
```
...
```sh
npx agentbot access add her@yourcompany.com
npx agentbot access add freelancer@example.com --chat-only
npx agentbot access list
```

An entry is an address or `@domain` (everyone at that domain). Admins get
the same access as a pairing code from `agentbot serve`; members get the
chat-only scope, the same as `agentbot pair --client`. The same lists live
...
The code itself comes from `accounts.agentbot.com`, the Agentbot
...
`agentbot sessions` and can be revoked the same way.
...
Plus one convenience: set `AGENTBOT_PUBLIC_URL=https://your.domain` so pairing
links, and `AGENTBOT_WEBHOOK_PUBLIC_URL=https://your.domain` so hook URLs, are
...
code that `agentbot serve` (or `agentbot pair`) prints, paste the
...
its own, listed and revocable with `agentbot sessions`. What it may do is
the code's scope: a code from `agentbot pair` carries `admin` and the app
shows everything; a code from `agentbot pair --client` (also what the
...
Both native apps pair this way. `agentbot pair --phone android` prints the
...
terminal, such as `docker compose exec omb node dist-server/agentbot.js
...
credentials, which are **not** `agentbot sessions` and are revoked from
...
For a foreground invocation, `npx --yes agentbot@X.Y.Z serve --tailscale`
...
git pull && pnpm install && sudo systemctl restart agentbot          # from source
```
