...
description: Build and verify the Windows desktop build (NSIS installer + latest.yml) for the canonical Agentbot release and its legacy updater mirror. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `Agentbot-<version>-setup.exe` and its update feed to
[milind-soni/Agentbot](https://github.com/milind-soni/Agentbot/releases).
...
| File | Purpose |
|---|---|
| `Agentbot-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `Agentbot-<version>-setup.exe.blockmap` | differential updates |
| `Agentbot-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `milind-soni/Agentbot` and, while the
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\Agentbot\logs\server.log`.
...
```powershell
Copy-Item release/Agentbot-<version>-setup.exe release/Agentbot-setup.exe
gh release upload v<version> --repo milind-soni/Agentbot `
  release/Agentbot-<version>-setup.exe `
  release/Agentbot-setup.exe `
  release/Agentbot-<version>-setup.exe.blockmap `
  release/latest.yml
```
...
to the matching draft in `milind-soni/agentbot-releases`; never replace the
bytes of an already-published asset.

Both names are required, for different consumers:

- **`Agentbot-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`Agentbot-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/Agentbot-setup.exe` button a stable URL. This
  mirrors `Agentbot.dmg` sitting beside `Agentbot-<version>.dmg`.
