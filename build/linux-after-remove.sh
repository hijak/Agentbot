...
if [ -n "${OPENMAUSBOT_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  case "$TEST_ROOT" in
    /tmp/*) ;;
    *) echo "Agentbot test install root must stay under /tmp" >&2; exit 1 ;;
  esac
...
if [ "$TEST_MODE" -eq 0 ]; then
  if command -v update-alternatives >/dev/null 2>&1; then
    update-alternatives --remove agentbot /opt/Agentbot/agentbot
  elif [ -L /usr/bin/agentbot ] && [ "$(readlink /usr/bin/agentbot)" = /opt/Agentbot/agentbot ]; then
    rm -- /usr/bin/agentbot
  fi
fi

profile=$APPARMOR_DIR/agentbot-browser
if [ ! -e "$profile" ] && [ ! -L "$profile" ]; then exit 0; fi
if [ -L "$APPARMOR_DIR" ] || [ -L "$profile" ] || [ ! -f "$profile" ]; then
  echo "Agentbot AppArmor profile is unsafe; refusing to remove it: $profile" >&2
  exit 1
fi
if [ "$TEST_MODE" -eq 0 ] && [ -x /usr/bin/ischroot ] && /usr/bin/ischroot; then
...
elif [ -r "$APPARMOR_PROFILES" ] && ! grep -q '^agentbot-browser ' "$APPARMOR_PROFILES"; then
  ...
  if ! "$APPARMOR_PARSER" -R "$profile"; then
    echo "Agentbot could not unload its browser AppArmor profile: $profile" >&2
    exit 1
  fi
elif [ -r "$APPARMOR_PROFILES" ]; then
  echo "Agentbot needs apparmor_parser to unload its browser profile: $profile" >&2
  exit 1
fi
rm -- "$profile"
