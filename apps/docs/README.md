...
```bash
pnpm docs:build
pnpm --filter @agentbot/docs types:check
pnpm --filter @agentbot/docs lint
```

The changelog reads published releases from `milind-soni/Agentbot` plus the
...
Create a second Vercel project beside the existing `agentbot.com` project:

1. Import the `milind-soni/Agentbot` repository.
...
5. Add `docs.agentbot.com` under **Settings → Domains**.
...
preview URLs for documentation pull requests. Keep `agentbot.com` on the
