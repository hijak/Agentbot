...
async function main() {
  ...
  const workspace = mkdtempSync(path.join(tmpdir(), "agentbot-update-smoke-"));
  ...
  const launched = path.join(installDir, "Agentbot-0.0.1-x86_64.AppImage");
  copyFileSync(packaged, launched);
  const desktopEntry = path.join(applications, "com.agentbot.app.desktop");
  writeFileSync(
    desktopEntry,
    `[Desktop Entry]\nName=Agentbot\nExec=${launched} %U\nType=Application\n`,
  );
