...
if (!podman || !machine) throw new Error("Set AGENTBOT_VERIFY_PODMAN and AGENTBOT_VERIFY_MACHINE to an explicit test engine");
const output = resolve(process.env.AGENTBOT_VERIFY_OUTPUT || ".omb-scratch/firefox");
...
process.env.AGENTBOT_DATA_DIR = resolve(fixtureHome, "app-data");
