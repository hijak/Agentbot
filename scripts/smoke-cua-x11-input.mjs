...
const prefix = "agentbot-cua-x11-input-";
...
const title = `Agentbot CUA input safety ${process.pid}`;
const xev = spawn(
  "xev",
...
try {
  ...
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "agentbot-x11-input-smoke", version: "1" },
  });
  ...
  const session = `agentbot-x11-smoke-${process.pid}`;
  assertToolResult(
