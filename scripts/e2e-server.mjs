// End-to-end check of a running Agentbot harness server — the exact
...
// codex turn, and — with --with-box + AGENTBOT_E2E_BOX_TOKEN — box provisioning,
...
const PORT = Number(opt("--port", process.env.AGENTBOT_PORT ?? 8799));
const BASE = `http://127.0.0.1:${PORT}`;
const WITH_BOX = flag("--with-box");
const KEEP_BOTS = flag("--keep-bots");
const BOX_TOKEN = process.env.AGENTBOT_E2E_BOX_TOKEN ?? "";
...
async function main() {
  ...
  try {
    ...
    // ── box: cloud computer ──
    if (WITH_BOX) {
      if (!BOX_TOKEN) fail("--with-box needs AGENTBOT_E2E_BOX_TOKEN");
