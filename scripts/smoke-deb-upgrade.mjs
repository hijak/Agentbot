...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "agentbot"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing Agentbot installation");
} catch (error) {
  if (String(error?.message ?? error).includes("refusing to replace")) throw error;
}

const temporary = fs.mkdtempSync(path.join(path.resolve(runnerTemp), "agentbot-deb-upgrade-"));
...
const legacyApp = path.join(legacyRoot, "opt", "Agentbot");
const legacyResources = path.join(legacyApp, "resources");
const legacyDeb = path.join(temporary, "agentbot_0.1.7_amd64.deb");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: Agentbot CI <ci@agentbot.invalid>",
      "Description: Legacy Agentbot directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/Agentbot", "/opt/Agentbot/resources"]) {
  ...
  for (const directory of [
    "/opt/Agentbot",
    "/opt/Agentbot/resources",
    "/opt/Agentbot/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/Agentbot/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/Agentbot/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "agentbot"], {
    encoding: "utf8",
  }).trim();
  console.log(
