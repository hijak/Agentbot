...
import { launchVerificationServer, runControlOmb, type VerificationServer } from "./control-agentbot.ts";
...
async function restartFixture(fixture: VerificationServer): Promise<ChildProcess> {
  ...
  Object.assign(env, {
    ...
    AGENTBOT_DATA_DIR: dataDir, AGENTBOT_PORT: new URL(fixture.info.url).port,
    AGENTBOT_WEBHOOK_PORT: String(Number(new URL(fixture.info.url).port) + 1),
  ...
  try {
    ...
    for (;;) {
      ...
      try {
        ...
        if (response.ok && health.app === "agentbot" && health.pid === child.pid) return child;
...
export async function verifyWorkspaceBackup(report: (event: unknown) => void = () => {}) {
  ...
  try {
    ...
    const clientState = { "agentbot-skin": "default", "agentbot-drafts": JSON.stringify({ [bot.threadId]: "Keep draft" }), "agentbot-webhook-credentials": "private-source-webhook-url", "unrelated-auth-token": "must-not-transfer" };
    ...
    assert.deepEqual(preferences.body.clientState, { "agentbot-skin": "default", "agentbot-drafts": clientState["agentbot-drafts"] });
