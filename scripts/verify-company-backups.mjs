...
const MARKER = "agentbot-pending-workspace-restore";
...
if (process.versions.electron && process.argv.includes(flag)) {
  ...
  handle("company-backups:create", input => {
    assert.equal(input.clientState["agentbot-drafts"], "fixture private draft");
  ...
  app.whenReady().then(async () => {
    ...
    await evaluate("localStorage.setItem('agentbot-drafts','fixture private draft'); localStorage.setItem('fixture-auth-token','must-stay-local');");
    ...
    await until(() => evaluate("document.body.textContent.includes('Fully quit Agentbot') && !document.querySelector('dialog[open]')"), "restart-required confirmation");
    ...
    await until(() => evaluate("document.body.textContent.includes('Fully quit Agentbot')"), "pending restore on reopen");
    ...
    assert.equal(await evaluate("localStorage.getItem('agentbot-drafts')"), "fixture private draft");
    ...
    for (const draft of ["new draft from scheduled day one", "changed draft from scheduled day two"]) {
      await evaluate(`localStorage.setItem('agentbot-drafts', ${JSON.stringify(draft)})`);
      ...
      assert.equal(preview.summary.format, "agentbot.workspace-backup");
      ...
      assert.equal(staged.clientState?.["agentbot-drafts"], draft);
  ...
  const { launchVerificationServer, runControlOmb } = await import("./control-agentbot.ts");
  const { waitForExit } = await import("../server/testing/cleanup.ts");
  const fixture = await launchVerificationServer({});
  let runtime = fixture.child, child, ui, restartTask;
  const output = mkdtempSync(join(tmpdir(), "agentbot-company-backup-ui-"));
  ...
  const restart = async () => {
    ...
    const env = { PATH: dirname(process.execPath), HOME: home, USERPROFILE: home, XDG_CONFIG_HOME: join(home, ".config"),
      ...
      TEMP: temporary, TMP: temporary, TMPDIR: temporary, HERMES_HOME: join(home, ".hermes"), AGENTBOT_DATA_DIR: fixture.info.dataDir,
      AGENTBOT_PORT: new URL(fixture.info.url).port, AGENTBOT_WEBHOOK_PORT: String(Number(new URL(fixture.info.url).port) + 1),
