...
import { launchUi, runControlOmbUi } from "./testing/control-agentbot-ui.ts";
...
export const HELP_UI = `renderer (needs a ui launch handle; every verb takes --ui HANDLE, never discovery):
  node --experimental-strip-types scripts/control-agentbot.ts ui launch [--entry threads] [--tool-calls JSON] [--mode happy]
...
export const HELP = `control-agentbot — verify a running Agentbot instance through its shared MCP core
...
mutating (an explicit --url or OPENMAUSBOT_URL/AGENTBOT_PORT is required):
...
export function parse(
  ...
  try {
    ...
    throw new ControlOmbError(
      ...
      `run control-agentbot help for the ${command} syntax`,
    );
  }
}
...
function configuredUrl(raw: unknown, env: NodeJS.ProcessEnv, requiredForMutation: boolean): string | undefined {
  const explicit = typeof raw === "string" && raw.trim()
    ? raw.trim()
    : env.OPENMAUSBOT_URL?.trim() || (env.AGENTBOT_PORT ? `http://127.0.0.1:${env.AGENTBOT_PORT}` : "");
  if (!explicit) {
    if (requiredForMutation) {
      throw new ControlOmbError(
        "mutating commands require an explicit Agentbot instance",
        "start `control-agentbot launch`, then pass its URL with --url",
      );
    }
    return undefined;
  }
...
export async function runControlOmb(
  ...
  throw new ControlOmbError(`unknown command ${JSON.stringify(command)}`, "run control-agentbot help");
}
...
export async function launchVerificationServer(
  ...
  const dataDir = mkdtempSync(join(browser && process.platform !== "win32" ? "/tmp" : tmpdir(), "agentbot-verify-data-"));
  ...
  const evidenceDir = join(tmpdir(), "agentbot-verification-evidence");
  ...
  if (localVm) Object.assign(childEnv, {
    AGENTBOT_EXTRA_PATH: [localVm.binDir, ...(process.platform === "win32" ? [join(childEnv.SYSTEMROOT || "C:\\Windows", "System32")] : [])].join(delimiter),
    CONTAINER_HOST: localVm.host,
    CONTAINER_SSHKEY: localVm.sshKey,
    AGENTBOT_STATIC_DIR: localVm.staticDir,
  });
  if (enterprise) Object.assign(childEnv, { AGENTBOT_ENTERPRISE_DIR: enterprise.dir, AGENTBOT_LICENSE_KEY: enterprise.licenseKey });
  if (browser) Object.assign(childEnv, {
    AGENTBOT_AGENT_BROWSER_PATH: browser.binaryPath,
    AGENT_BROWSER_EXECUTABLE_PATH: browser.executablePath,
  });
  if (boxFixtureApi) childEnv.AGENTBOT_BOX_API = boxFixtureApi;
...
function requireForegroundTerminal(command: string): void {
  if (process.env.npm_lifecycle_event === "control:omb") {
    throw new ControlOmbError(
      ...
      `run \`node --experimental-strip-types scripts/control-agentbot.ts ${command}\``,
    );
  }
}
