...
const home = mkdtempSync(join(tmpdir(), "agentbot-opencode-permissions-"));
const workspace = join(home, "workspace");
const external = join(home, "outside", "receipt.txt");
mkdirSync(workspace);
mkdirSync(join(home, "outside"));
writeFileSync(external, "AGENTBOT_EXTERNAL_READ_RECEIPT");
process.env = {
  ...
  AGENTBOT_DATA_DIR: join(home, "omb"), OPENCODE_DISABLE_DEFAULT_PLUGINS: "true",
...
const server = createServer(async (req, res) => {
  ...
  readSucceeded ||= toolResult && JSON.stringify(payload.messages.at(-1)).includes("AGENTBOT_EXTERNAL_READ_RECEIPT");
