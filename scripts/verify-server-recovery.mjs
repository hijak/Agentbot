...
if (process.versions.electron && process.argv.includes(flag)) {
  ...
  app.whenReady().then(async () => {
    ...
    const leaseFile = join(dataDir, "agentbot-server.lease");
    ...
    const start = async () => {
      const proc = utilityProcess.fork(join(output, "server/index.js"), [], {
        ...
        env: {
          ...
          AGENTBOT_DATA_DIR: dataDir, AGENTBOT_PORT: String(port), AGENTBOT_WEBHOOK_PORT: String(webhookPort),
          AGENTBOT_STATIC_DIR: join(output, "ui"), AGENTBOT_DESKTOP_PARENT: "1",
      ...
      proc.once("spawn", () => {
        ...
        proc.postMessage({ type: "agentbot:desktop-mutation-token", token, companionToken: token });
      });
    ...
    if (scenario === "recover") {
      ...
      assert.equal(JSON.parse(readFileSync(join(dataDir, ".agentbot-server-child/agentbot-server.lease"), "utf8")).pid, replacementPid);
  ...
  const output = mkdtempSync(join(process.platform === "darwin" ? "/tmp" : tmpdir(), "agentbot-server-recovery-"));
