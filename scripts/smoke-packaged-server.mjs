...
const staging = mkdtempSync(join(tmpdir(), "agentbot-smoke-"));
const home = mkdtempSync(join(tmpdir(), "agentbot-smoke-home-"));
const port = 21000 + Math.floor(Math.random() * 9000);

// AGENTBOT_SMOKE_DIST lets the release workflow aim this at a packaged app's
...
const fixtureEnv = {
  ...
  ...(browserBundle ? {
    AGENTBOT_RESOURCES_PATH: staging,
...
if (
  ...
  mcpReport.responses?.find((response) => response.id === 1)?.result?.serverInfo?.name !== "agentbot-mcp" ||
