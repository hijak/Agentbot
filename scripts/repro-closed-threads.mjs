...
// close_thread stay in the sidebar. Runs an isolated Agentbot server on a
...
const home = mkdtempSync(join(tmpdir(), "agentbot-closed-threads-repro-"));
...
const child = spawn(process.execPath, ["--experimental-strip-types", join(ROOT, "server", "index.ts")], {
  ...
  env: {
    ...
    HOME: home, USERPROFILE: home, AGENTBOT_DATA_DIR: home,
    TMPDIR: home,
    AGENTBOT_PORT: String(port), AGENTBOT_WEBHOOK_PORT: String(port + 1),
    AGENTBOT_TEST_INTERNAL_CAPABILITY_KEY: TEST_KEY,
    FAKE_CLAUDE_MODE: "happy",
    ...(existsSync(STATIC_DIR) ? { AGENTBOT_STATIC_DIR: STATIC_DIR } : {}),
  },
...
const minted = await api("POST", "/api/testing/internal-capability", { botId: parker.id, threadId: parker.threadId, kind: "agents", depth: 0 }, { "x-agentbot-test-capability": TEST_KEY });
