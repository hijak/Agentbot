...
const root = mkdtempSync(join(tmpdir(), 'agentbot-native-instructions-'));
process.env.AGENTBOT_DATA_DIR = join(root, 'omb');
...
async function start() {
    ...
    await rpc('initialize', { clientInfo: { name: 'agentbot-instruction-probe', version: '1' } });
...
try {
    ...
    if (!process.env.PROBE_BEFORE) {
        ...
        assert(text(8, 'developer').includes('No Agentbot bot-specific instructions remain.'));
