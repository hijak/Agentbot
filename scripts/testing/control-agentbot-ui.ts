// `control-agentbot ui`: drive the real React renderer headlessly against the
...
//
// Imported by scripts/control-agentbot.ts, which owns HELP and the MUTATING set;
...
/** AGENTBOT_AGENT_BROWSER_PATH, then the tools directory, then PATH; otherwise the
...
export async function ensureUiBrowser(
  ...
  if (!chrome) {
    const started = Date.now();
    await ensureChrome(binary, { env: sessionEnv({ home: toolsDir, session: "agentbot-ui-install", chrome: null }, parentEnv), log });
...
export async function runControlOmbUi(args: string[]): Promise<unknown> {
  ...
  if (verb === "launch") {
    throw new ControlOmbError("ui launch is available only from the executable CLI", "run `node --experimental-strip-types scripts/control-agentbot.ts ui launch`");
  }
  ...
  throw new ControlOmbError(`unknown ui command ${JSON.stringify(verb)}`, "run control-agentbot ui help");
}
