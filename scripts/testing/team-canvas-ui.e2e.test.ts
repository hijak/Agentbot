...
if (!enabled) console.info("skipping team canvas UI e2e: set AGENTBOT_UI_E2E=1 to install the pinned browser");
...
(enabled ? it : it.skip)("edits and arranges the team canvas without losing bots or conversations", async () => {
  ...
  try {
    ...
    const layout = await evaluate("Object.fromEntries(Object.entries(localStorage).filter(([key]) => key.startsWith('agentbot-team-canvas:')))");
    ...
    expect(await evaluate("Object.fromEntries(Object.entries(localStorage).filter(([key]) => key.startsWith('agentbot-team-canvas:')))"))
      .toEqual(layout);
    expect((await savedBot(ben.id)).section).toBe("Delivery");
    expect(await messages(ben.id)).toEqual(transcript);
