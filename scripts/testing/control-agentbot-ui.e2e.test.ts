...
// machine. It spawns the real `control-agentbot ui launch` (a child it can Ctrl-C),
...
// directory, AGENTBOT_AGENT_BROWSER_PATH or PATH) or when AGENTBOT_UI_E2E=1 asks for the
...
const CLI = join(ROOT, "scripts", "control-agentbot.ts");
...
if (!enabled) {
  console.log(`skipping control-agentbot ui e2e: no agent-browser binary resolves (looked in ${UI_TOOLS_DIR}, AGENTBOT_AGENT_BROWSER_PATH and PATH); set AGENTBOT_UI_E2E=1 to install the pinned release`);
}
...
// AGENTBOT_UI_EVIDENCE_DIR keeps the screenshot (CI uploads it); otherwise it is temporary.
const evidenceDir = process.env.AGENTBOT_UI_EVIDENCE_DIR ? resolve(ROOT, process.env.AGENTBOT_UI_EVIDENCE_DIR) : mkdtempSync(join(tmpdir(), "agentbot-ui-evidence-"));
const ownsEvidenceDir = !process.env.AGENTBOT_UI_EVIDENCE_DIR;
...
describe("control-agentbot ui drives the real renderer", () => {
