...
import { ensureUiBrowser, sessionEnv, UI_TOOLS_DIR } from "./control-agentbot-ui.ts";
import { fixtureApi, mountPreview, type MountedPreview } from "./preview-fixture.ts";

const enabled = process.env.AGENTBOT_UI_E2E === "1" || !!resolveAgentBrowserBinary({ dataDir: UI_TOOLS_DIR, env: process.env });
if (!enabled) console.info("skipping stream-buffer e2e: no agent-browser resolves; set AGENTBOT_UI_E2E=1 to install the pinned tools");
