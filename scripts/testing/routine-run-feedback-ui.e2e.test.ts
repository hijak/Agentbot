...
(process.env.AGENTBOT_UI_E2E === "1" ? it : it.skip)("Run now shows pending, its exact result, and retryable request errors", async () => {
  const root = fileURLToPath(new URL("../..", import.meta.url));
  let output = "";
  child = spawn(process.execPath, ["--experimental-strip-types", "scripts/control-agentbot.ts", "ui", "launch"], { cwd: root, env: process.env, stdio: ["ignore", "pipe", "pipe"] });
  ...
  if (process.env.AGENTBOT_RUN_FEEDBACK_SCREENSHOT) await ui("screenshot", "--out", process.env.AGENTBOT_RUN_FEEDBACK_SCREENSHOT);
}, 180_000);
