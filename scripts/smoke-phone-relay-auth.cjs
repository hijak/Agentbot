...
const home = mkdtempSync(join(tmpdir(), "agentbot-phone-auth-smoke-"));
...
app.whenReady().then(async () => {
  ...
  const env = {
    HOME: home, USERPROFILE: home, AGENTBOT_DATA_DIR: home, PATH: "",
    ...(process.env.SystemRoot ? { SystemRoot: process.env.SystemRoot } : {}),
    AGENTBOT_PORT: String(harnessPort), AGENTBOT_WEBHOOK_PORT: String(webhookPort),
    AGENTBOT_COMPANION_PORT: String(phonePort), AGENTBOT_CONTROL_PORT: String(controlPort),
    FAKE_CLAUDE_MODE: "happy",
  };
  const harness = fork("dist-server/index.js", { ...env, AGENTBOT_DESKTOP_PARENT: "1" });
  await until(() => api(harnessPort, "/api/health").catch(() => null));
  assert.equal((await api(harnessPort, "/api/bots", "POST", {})).status, 403);
  harness.postMessage({ type: "agentbot:desktop-mutation-token", token: owner, companionToken: relay });
  ...
  assert.equal((await api(harnessPort, "/api/bots", "POST", {}, {
    "x-agentbot-companion": "1", "x-agentbot-companion-device": paired.body.device.id,
  })).status, 403);
  sidecar.postMessage({ type: "agentbot:companion-mutation-token", token: relay });
  ...
  const created = await phone("/api/bots", "POST", { modelSelection: { instanceId: "claude", model: "claude-sonnet-5" } }, {
    "x-agentbot-companion-auth": "forged", "x-agentbot-desktop-owner": "forged",
    "x-agentbot-companion-device": "forged-device",
  });
  ...
  harness.postMessage({ type: "agentbot:desktop-mutation-token", token: owner, companionToken: nextRelay });
  const restarted = fork("dist-companion/index.js", env);
  restarted.once("spawn", () => restarted.postMessage({ type: "agentbot:companion-mutation-token", token: nextRelay }));
