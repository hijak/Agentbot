...
const image = process.env.AGENTBOT_VERIFY_IMAGE;
if (!podman || !machine || !image) throw new Error("Set explicit AGENTBOT_VERIFY_PODMAN, AGENTBOT_VERIFY_MACHINE and AGENTBOT_VERIFY_IMAGE");
const output = resolve(process.env.AGENTBOT_VERIFY_OUTPUT || ".omb-scratch/japanese");
...
process.env.AGENTBOT_DATA_DIR = resolve(runtime, "app-data");
