...
import { launchVerificationServer, runControlOmb } from "./control-agentbot.ts";

const podman = process.env.AGENTBOT_VERIFY_PODMAN;
const machine = process.env.AGENTBOT_VERIFY_MACHINE;
if (!podman || !isAbsolute(podman) || !machine) {
  throw new Error("Set AGENTBOT_VERIFY_PODMAN to an absolute executable path and AGENTBOT_VERIFY_MACHINE explicitly");
}
...
try {
  ...
  for (const id of ids) {
    ...
    const gate = await fetch(computer.env.AGENTBOT_CONTROL_URL, {
      headers: { authorization: `Bearer ${computer.env.AGENTBOT_CONTROL_TOKEN}` }, signal: AbortSignal.timeout(5_000),
    });
