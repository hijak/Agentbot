...
import { ensureUiBrowser } from "./testing/control-agentbot-ui.ts";
import { fixtureApi, mountPreview, parkUntilSignal } from "./testing/preview-fixture.ts";

const cli = process.env.AGENTBOT_VERIFY_CODEX_CLI;
const auth = process.env.AGENTBOT_VERIFY_CODEX_AUTH;
const model = process.env.AGENTBOT_VERIFY_CODEX_MODEL;
if (!cli || !auth || !model) throw new Error("Supply AGENTBOT_VERIFY_CODEX_CLI, AGENTBOT_VERIFY_CODEX_AUTH and AGENTBOT_VERIFY_CODEX_MODEL explicitly. This test uses real model quota.");
