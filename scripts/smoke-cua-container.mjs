...
const runtime = process.env.AGENTBOT_CUA_RUNTIME || "docker";
const container = process.env.AGENTBOT_CUA_CONTAINER;
const socket = process.env.AGENTBOT_CUA_SOCKET || "/run/user/1000/agentbot-cua.sock";
if (!container || !/^[a-zA-Z0-9_.-]+$/.test(container)) {
  throw new Error("set AGENTBOT_CUA_CONTAINER to the explicitly created smoke container name");
}
...
try {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "agentbot-container-smoke", version: "1" },
  });
