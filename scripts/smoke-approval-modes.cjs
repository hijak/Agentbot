...
const home = mkdtempSync(join(tmpdir(), "agentbot-approval-smoke-"));
...
app.whenReady().then(async () => {
  ...
  child = utilityProcess.fork(join(root, "server/index.ts"), [], {
    ...
    env: {
      ...
      AGENTBOT_PORT: String(port), AGENTBOT_WEBHOOK_PORT: String(webhookPort),
      AGENTBOT_TEST_INTERNAL_CAPABILITY_KEY: testCapabilityKey,
  ...
  if (process.argv.includes("--skill-ui-only")) {
    await require("./testing/skill-approval-ui-smoke.cjs")({ root, home, url: `http://127.0.0.1:${port}`, api, until,
      capability: (botId, threadId) => api("/api/testing/internal-capability", "POST", { botId, threadId, skillAuthoring: true }, { "x-agentbot-test-capability": testCapabilityKey }),
    });
    return;
  }
  ...
  const capability = await api("/api/testing/internal-capability", "POST", { botId: id, threadId: created.body.bot.threadId }, { "x-agentbot-test-capability": testCapabilityKey });
