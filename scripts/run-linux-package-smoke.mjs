...
const prefixName = "agentbot-linux-smoke-runtime-";
...
const executables = [
  path.join(root, "release", "linux-unpacked", "agentbot"),
  path.join(root, "release", appImage),
];
if (process.env.AGENTBOT_SMOKE_INSTALLED_DEB === "1") {
  executables.push("/opt/Agentbot/agentbot");
}
  ...
  const bundled = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...
        AGENTBOT_SMOKE_BUNDLED_CUA: "1",
        AGENTBOT_SMOKE_EXECUTABLE: executable,
      },
      stdio: "inherit",
    },
  );
  ...
  const signalShutdown = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...
        XDG_RUNTIME_DIR: runtimeDirectory,
        AGENTBOT_SMOKE_BUNDLED_CUA: "1",
        AGENTBOT_SMOKE_SIGNAL_SHUTDOWN: "1",
        AGENTBOT_SMOKE_EXECUTABLE: path.join(root, "release", appImage),
      },
      stdio: "inherit",
    },
  );
  ...
  const hardDeath = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...process.env,
        XDG_RUNTIME_DIR: runtimeDirectory,
        AGENTBOT_SMOKE_HARD_DEATH: "1",
        AGENTBOT_SMOKE_EXECUTABLE: path.join(root, "release", "linux-unpacked", "agentbot"),
      },
      stdio: "inherit",
    },
  );
  ...
  const result = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...process.env,
        XDG_RUNTIME_DIR: runtimeDirectory,
        AGENTBOT_SMOKE_WAYLAND: lane.wayland ? "1" : "0",
        AGENTBOT_SMOKE_LINUX_CUA_BLOCKED: lane.blocked ? "1" : "0",
      },
      stdio: "inherit",
    },
  );
