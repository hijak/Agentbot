...
for kind in iphone ipad; do
  simulator_id=$(xcrun simctl create "agentbot-ios-threads-${kind}-${GITHUB_RUN_ID}" "$(device_type "$kind")" "$runtime_id")
  created_ids+=("$simulator_id")
  xcodebuild \
    -project AgentbotCompanion.xcodeproj \
    ...
    -only-testing:AgentbotCompanionUITests/ThreadNavigationUITests \
