#!/usr/bin/env node
// Model Context Protocol (MCP) Server for Agentbot
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid Agentbot URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("Agentbot URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("Agentbot URL must not contain credentials; use OPENMAUSBOT_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("Agentbot URL must be an origin without a path, query, or fragment");
  }
...
const configuredUrl = process.env.OPENMAUSBOT_URL ||
  (process.env.AGENTBOT_PORT ? `http://127.0.0.1:${process.env.AGENTBOT_PORT}` : undefined);

export const AGENTBOT_BASE_URL = validateBaseUrl(configuredUrl || "http://127.0.0.1:8799");
...
export function log(msg: string) {
  process.stderr.write(`[agentbot-mcp] ${msg}\n`);
}
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 && !process.env.OPENMAUSBOT_TOKEN?.trim()) {
      throw new Error(
        "Agentbot refused this write because the installed desktop app requires a paired session token. " +
        "Set OPENMAUSBOT_TOKEN as described in docs/mcp-server.md.",
      );
    }
    throw new Error(`Agentbot API error (${response.status}): ${text || response.statusText}`);
  }
  try {
    return await response.json();
  } catch {
    throw new Error(`Agentbot API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "agentbot") {
        failures.push(`${candidate} answered, but it was not Agentbot`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running Agentbot server. ${failures.join("; ")}`);
}

export async function resolveBaseUrl(): Promise<string> {
  if (discoveredBaseUrl) return discoveredBaseUrl;
  if (process.env.OPENMAUSBOT_TOKEN?.trim() && !configuredUrl) {
    throw new Error("Set OPENMAUSBOT_URL or AGENTBOT_PORT when using OPENMAUSBOT_TOKEN so credentials are never sent during port discovery");
  }
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the Agentbot server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid Agentbot ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "agentbot") throw new Error("The configured endpoint is not an Agentbot server");
      return {
        status: "connected",
        endpoint: discoveredBaseUrl ?? AGENTBOT_BASE_URL,
        app: "agentbot",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("Agentbot did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("Agentbot did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("Agentbot did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("Agentbot did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("Agentbot did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("Agentbot did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
    ...
    case "set_bot_model": {
      ...
      if (args.task_id !== undefined) {
        ...
        if (!isRecord(res?.task)) throw new Error("Agentbot did not return the updated task");
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        instructions: "Use bounded read tools before mutating the Agentbot team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("Agentbot MCP server running on stdio");
}
