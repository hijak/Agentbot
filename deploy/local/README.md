...
`AGENTBOT_HTTP_PORT` changes the host port and the default public URL.
...
For Tailscale Serve, set `AGENTBOT_PUBLIC_URL` to your HTTPS URL and
`AGENTBOT_HTTPS_HOST` to its hostname without scheme or path. Configure Tailscale
Serve on the host to forward to the chosen localhost HTTP port.
Keep the default `AGENTBOT_BIND_ADDRESS=127.0.0.1`. Caddy refuses to start when
`AGENTBOT_HTTPS_HOST` is set with any other bind address, so direct remote HTTP
...
Without `AGENTBOT_HTTPS_HOST`, the bind address can be changed for HTTP access.
...
docker compose exec omb node dist-server/agentbot.js pair
```
...
volume, set `AGENTBOT_DATA_VOLUME` to its name and `AGENTBOT_DATA_EXTERNAL=true`.
