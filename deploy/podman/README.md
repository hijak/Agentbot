...
Run the Agentbot server, Caddy, and per-bot Linux desktops with one Podman
...
```text
Browser -> loopback Caddy :8080 -> Agentbot :8799
                                    |
...
Setup creates/starts a WSL2 machine named `agentbot` (4 CPUs, 10 GiB RAM,
...
It preserves an existing `.env`. Use `AGENTBOT_PODMAN_MACHINE` to select a different
machine. The wrapper refuses a stopped machine for normal Compose commands;
after a reboot, use `podman machine start agentbot` before `up -d`.

Compose runs **inside** the machine, so the socket and bind paths have the same
meaning for the server and the engine. `AGENTBOT_PODMAN_ENV_FILE` selects an alternate
...
for example `maus.ps1 exec omb claude`. Their logins persist under `AGENTBOT_DATA_ROOT`.
...
endpoint to `http://127.0.0.1:8080`, set `AGENTBOT_PUBLIC_URL` to that HTTPS URL and
`AGENTBOT_HTTPS_HOST` to its hostname, then recreate the services with `up -d`.
Mint a pairing code with `maus.ps1 exec omb node dist-server/agentbot.js pair`.
...
- Data, chats, credentials, and desktop workspaces live under `AGENTBOT_DATA_ROOT`.
