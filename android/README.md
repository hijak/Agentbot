...
**Agentbot Preview**, its application ID is `com.agentbot.companion.preview`,
...
not register the release's `agentbot://` pairing links or system share targets.
...
```sh
keytool -genkeypair -v -keystore ~/agentbot-release.jks \
  -storetype PKCS12 -alias agentbot -keyalg RSA -keysize 4096 -validity 10000
```
...
"$APKSIGNER" sign --ks ~/agentbot-release.jks --ks-key-alias agentbot \
...
```properties
storeFile=/absolute/path/to/agentbot-release.jks
storePassword=…
keyAlias=agentbot
keyPassword=…
```
