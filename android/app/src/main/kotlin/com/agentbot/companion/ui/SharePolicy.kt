package com.agentbot.companion.ui

import com.agentbot.companion.core.APIError
import com.agentbot.companion.core.Bot
import com.agentbot.companion.core.CompanionClient
import com.agentbot.companion.core.CompanionEndpointKind
import com.agentbot.companion.core.Connection
import com.agentbot.companion.core.ConnectionAdvice
import com.agentbot.companion.core.Fleet
import com.agentbot.companion.core.MessageDestination
import com.agentbot.companion.core.Room
import java.net.URI
...
object SharePolicy {
    ...
    fun friendlyMessage(error: Throwable, computerName: String?): String {
        ...
        if (api != null && isAmbiguousTransport(api)) {
            return computerName?.let(::offline) ?: "Couldn't reach your computer. Keep Agentbot open and Phone access on, then try again."
        }
        ...
        detail == "Unlock this phone, then try sharing again." ||
            ...
            (
                detail.startsWith("Couldn't reach ") &&
                    detail.endsWith(". Keep Agentbot open and Phone access on, then try again.")
                )
    ...
    fun notPaired(): String =
        "Open the Agentbot app once after updating. If this phone still isn't connected, pair it before sharing."
    ...
    fun imageSupportUnavailable(): String =
        "Update Agentbot on this computer before sharing images."

    fun offline(name: String): String =
        "Couldn't reach $name. Keep Agentbot open and Phone access on, then try again."
    ...
    fun unreadable(name: String): String =
        "Agentbot couldn't read $name. Try exporting it to Files first."

    fun sendTimedOut(): String = "Sending took too long. Check your connection and try again."

    fun generic(): String = "Agentbot couldn't send this. Please try again."

    fun pairingExpired(): String =
        "This phone's pairing has expired. Open Agentbot and pair it again."
