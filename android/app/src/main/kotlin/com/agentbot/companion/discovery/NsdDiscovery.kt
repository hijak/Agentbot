...
internal fun browseDiscoveryFlow(
    ...
    fun newListener(): NsdManager.DiscoveryListener = object : NsdManager.DiscoveryListener {
        ...
        override fun onServiceFound(service: NsdServiceInfo) {
            if (service.serviceType != serviceType && !service.serviceType.contains("agentbot")) {
                return
            }
            attemptResolve(service, attemptsSoFar = 1)
        }
...
/**
 * NsdManager wrapper for `_agentbot._tcp`, exposed as a Flow of [DiscoveryState].
 */
class NsdDiscovery(
    context: Context,
    ...
    companion object {
        const val SERVICE_TYPE = "_agentbot._tcp."
        const val MULTICAST_LOCK_TAG = "agentbot-nsd"
    }
}
