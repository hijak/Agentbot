...
export function createComputerSharing({ file, fetch: fetchImpl, environments, cuaConnection, hostControl, protectedPaths = [], enabled = async () => false }) {
  ...
  const request = async (env, route, body, signal, secret) => {
    ...
    let json; try { json = JSON.parse(Buffer.concat(chunks).toString("utf8")); } catch { throw new Error("This server does not support computer sharing. Update its Agentbot installation."); }
  ...
  const identity = async env => {
    ...
    const [auth, descriptor] = await Promise.all([request(env, "/api/auth/session"), request(env, "/.well-known/agentbot/environment")]);
    await requireEnabled();
