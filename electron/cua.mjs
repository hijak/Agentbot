...
//    EmbeddedCuaDriverHost so TCC grants attribute to Agentbot and the
//    driver inherits them. One prompt, named Agentbot, out of the box.
...
const HOST_BUNDLE_ID = "com.agentbot.app";
...
async function attachStandalone(signal) {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed Agentbot.
...
async function startEmbedded(binary, signal) {
  ...
  // Agentbot rather than to a terminal or helper process.
  if (process.platform === "darwin") {
    ...
    if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
      ...
      throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart Agentbot`);
    }
  }
