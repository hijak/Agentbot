...
describe("workspace credential migration", () => {
  it("stores the router key separately while keeping provider settings", () => {
    ...
    expect(workspaceCredentialEnv(result.credentials)).toEqual({ AGENTBOT_OPENAI_IMAGE_KEY: "openai-only", AGENTBOT_CUSTOM_IMAGE_KEY: "router-only" });
  });
...
describe("workspace credential env", () => {
  it("maps each stored secret to exactly its server env var", () => {
    expect(
      ...
      AGENTBOT_TTS_KEY: "tts-secret",
      AGENTBOT_FISH_AUDIO_API_KEY: "fish-secret",
      OPENCODE_API_KEY: "ocg-secret",
      AGENTBOT_OPENAI_IMAGE_KEY: "image-secret",
    });
  });
