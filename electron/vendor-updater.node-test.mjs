...
test("the shipped installer replaces the AppImage and queues its original path for relaunch", (t) => {
  ...
  const workspace = mkdtempSync(join(tmpdir(), "agentbot-appimage-install-"));
  ...
  const launched = join(workspace, "Agentbot-0.1.43-x86_64.AppImage");
  const staged = join(workspace, "pending", "Agentbot-0.1.44-x86_64.AppImage");
  ...
  assert.deepEqual(readdirSync(workspace).sort(), ["Agentbot-0.1.43-x86_64.AppImage", "data", "pending"]);
...
test("a rejected AppImage relaunch is reported without quitting the desktop", (t) => {
  ...
  const workspace = mkdtempSync(join(tmpdir(), "agentbot-appimage-relaunch-failed-"));
  t.after(() => rmSync(workspace, { recursive: true, force: true }));
  const launched = join(workspace, "Agentbot.AppImage");
...
test("the running AppImage is never removed before its replacement is in place", (t) => {
  ...
  const { AppImageUpdater } = createRequire(import.meta.url)("./vendor/electron-updater.cjs");
  const workspace = mkdtempSync(join(tmpdir(), "agentbot-appimage-failed-"));
  t.after(() => rmSync(workspace, { recursive: true, force: true }));
...
test("Electron relaunch waits for deferred cleanup and the replacement acquires the same data directory", {
  ...
  const workspace = mkdtempSync(join(tmpdir(), "agentbot-appimage-relaunch-"));
  t.after(() => rmSync(workspace, { recursive: true, force: true }));
  const launched = join(workspace, "Agentbot-1.0.0.AppImage");
  const staged = join(workspace, "Agentbot-2.0.0.AppImage");
  ...
  const replacement = `
    ...
    (async () => {
      ...
      try { process.kill(Number(process.env.AGENTBOT_UPDATER_TEST_PARENT), 0); }
  ...
  writeFileSync(fixture, `
    ...
    app.whenReady().then(async () => {
      ...
      process.env.AGENTBOT_UPDATER_TEST_PARENT = String(process.pid);
