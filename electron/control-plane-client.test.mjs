...
describe("control-plane desktop client", () => {
  it("accepts exact HTTPS and loopback development origins only", () => {
    expect(normalizeControlPlaneURL("https://accounts.agentbot.com/")).toBe(
      "https://accounts.agentbot.com",
    );
    expect(normalizeControlPlaneURL("http://127.0.0.1:8787/")).toBe("http://127.0.0.1:8787");
    expect(normalizeControlPlaneURL("http://accounts.agentbot.com")).toBe("");
    expect(normalizeControlPlaneURL("https://accounts.agentbot.com/api")).toBe("");
    expect(normalizeControlPlaneURL("https://user:secret@accounts.agentbot.com")).toBe("");
  });
  ...
  it("normalizes an email without accepting malformed input", () => {
    ...
    expect(normalizeControlPlaneURL({ toString: () => "https://accounts.agentbot.com" })).toBe("");
  });
  ...
  it("accepts plain cross-realm response records", async () => {
    ...
    const client = createControlPlaneClient({
      baseURL: "https://accounts.agentbot.com",
  ...
  it("requires the exact healthy control-plane identity before onboarding", async () => {
    ...
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ ok: true, service: "agentbot-control-plane" }))
    ...
    expect(fetchImpl.mock.calls[0][0]).toBe("https://accounts.agentbot.com/healthz");
  ...
  it("identifies native Better Auth mutations with the trusted control-plane origin", async () => {
    const fetchImpl = vi.fn(async (_url, init) => {
      ...
      if (wireHeaders.has("sec-fetch-mode") && wireHeaders.get("origin") !== "https://accounts.agentbot.com") {
  ...
  it("keeps a valid installation credential without rotating it", async () => {
    const fetchImpl = vi.fn(async (url) => {
      expect(url).toBe("https://accounts.agentbot.com/v1/installations/self");
      return jsonResponse({
        installation: {
          id: INSTALL_ID,
    ...
    const client = createControlPlaneClient({ baseURL: "https://accounts.agentbot.com", fetchImpl });
  ...
  it("lists validated active installations for response-loss cleanup", async () => {
    const fetchImpl = vi.fn(async (url, init) => {
      expect(url).toBe("https://accounts.agentbot.com/v1/installations");
  ...
  it("validates endpoint material without leaking the connector token into the URL", async () => {
    ...
    const fetchImpl = vi.fn(async (url, init) => {
      expect(url).toBe("https://accounts.agentbot.com/v1/installations/self/endpoint");
      expect(init.headers.get("authorization")).toBe(`Bearer ${INSTALL}`);
      expect(url).not.toContain(connectorToken);
      return jsonResponse({ endpoint: { url: "https://c-opaque.agentbot.com" }, connectorToken });
    });
    const client = createControlPlaneClient({ baseURL: "https://accounts.agentbot.com", fetchImpl });
    await expect(client.ensureEndpoint(INSTALL)).resolves.toEqual({
      endpoint: { url: "https://c-opaque.agentbot.com" },
      connectorToken,
    });
  });
