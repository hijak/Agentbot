...
export function managedComposioChildEnvironment(brokerUrl, credentials, environment) {
  ...
  delete next.AGENTBOT_COMPOSIO_BROKER_URL;
  delete next.AGENTBOT_COMPOSIO_BROKER_TOKEN;
  const access = managedComposioAccess(brokerUrl, credentials);
  if (access) {
    next.AGENTBOT_COMPOSIO_BROKER_URL = access.url;
    next.AGENTBOT_COMPOSIO_BROKER_TOKEN = access.token;
  }
  return next;
}
