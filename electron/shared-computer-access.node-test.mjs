...
async function fixture(t) {
  const dir = await realpath(await mkdtemp(path.join(tmpdir(), "agentbot-shared-access-")));
...
function stubWorkspace() {
  ...
  const fetchImpl = async (url, init) => {
    ...
    if (route === "/.well-known/agentbot/environment") return json({ environmentId, capabilities: { sharedComputers: true } });
...
test("a protected directory spelled in another case is still refused", async t => {
  ...
  await mkdir(path.join(dir, "Agentbot"));
  await writeFile(path.join(dir, "Agentbot", "credentials.bin"), "credential blob");
  grant.protectedPaths = [path.join(dir, "Agentbot")];
  folder.write = true;
  await assert.rejects(run({ action: "read_file", path: "Agentbot/credentials.bin" }), /Desktop credentials/);
  await assert.rejects(run({ action: "read_file", path: "agentbot/credentials.bin" }), /Desktop credentials/);
  await assert.rejects(run({ action: "read_file", path: "OPENMAUSBOT/credentials.bin" }), /Desktop credentials/);
  await assert.rejects(run({ action: "list_files", path: "agentbot" }), /Desktop credentials/);
  await assert.rejects(run({ action: "write_file", path: "agentbot/computer-sharing.json", content: "{}" }), /sharing settings/);
});
...
test("a harness data directory that does not exist yet still saves and connects", async t => {
  ...
  const sharing = createComputerSharing({
    file: path.join(dir, "profile", "computer-sharing.json"), fetch: stub.fetchImpl,
    environments: () => [stub.env], enabled: async () => true, cuaConnection: async () => null, protectedPaths: [path.join(dir, "never-installed", ".agentbot")],
  });
  t.after(() => sharing.close());
  const info = await sharing.identity(stub.env);
