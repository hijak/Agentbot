...
test("the Ubuntu command resolves dependencies", () => {
  const command = packageInstallCommand("deb", "/home/u/.cache/agentbot-updater/pending/x.deb");

  assert.equal(
    command,
    "sudo apt-get install -y '/home/u/.cache/agentbot-updater/pending/x.deb'",
  );
...
test("a staged path that is gone is not used", () => {
  const workspace = mkdtempSync(join(tmpdir(), "agentbot-staged-"));
  try {
    const present = join(workspace, "Agentbot.deb");
...
test("the quoted path survives a shell round-trip", { skip: posixShell }, () => {
  const workspace = mkdtempSync(join(tmpdir(), "agentbot-quote-"));
  try {
...
test("the whole command parses into the arguments apt-get would receive", { skip: posixShell }, () => {
  const file = "/home/o'brien/.cache/agentbot-updater/pending/Agentbot-0.1.44-amd64.deb";
...
test("the package marker decides the install path", () => {
  ...
  assert.equal(
    linuxPackageType({
      ...
      appImage: "/home/u/Agentbot.AppImage",
