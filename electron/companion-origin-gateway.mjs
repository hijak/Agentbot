...
const RUNTIME_PREFIX = "agentbot-companion-origin-";
...
export function validCompanionOriginTarget(target, platform = process.platform) {
  ...
  if (platform === "win32") {
    return /^\\\\\.\\pipe\\agentbot-companion-origin-[1-9][0-9]*-[0-9a-f-]{36}$/i.test(
      target.socketPath,
    );
  }
  return (
...
export function createCompanionOriginEndpoint({
  ...
  if (platform === "win32") {
    return Object.freeze({
      pid: processId,
      socketPath: `\\\\.\\pipe\\agentbot-companion-origin-${processId}-${identifier()}`,
      directory: null,
    });
  }
    ...
    const outgoing = request(
      ...
      (response) => {
        ...
        response.on("end", () => {
          ...
          try {
            finish(JSON.parse(Buffer.concat(chunks).toString("utf8"))?.app === "agentbot");
          } catch {
            finish(false);
          }
        });
