...
const canSymlink = (() => {
  const probe = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-symlink-probe-"));
  try {
...
before(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-save-file-"));
  botHome = path.join(home, ".agentbot");
...
describe("save-file path validation", () => {
  ...
  it("accepts a file under a symlinked bot home", { skip: !canSymlink }, async () => {
    const realHome = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-real-home-"));
    const linkedHome = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-linked-home-"));
    ...
    fs.symlinkSync(realBotHome, path.join(linkedHome, ".agentbot"));

    const viaLink = path.join(linkedHome, ".agentbot", "report.docx");
...
describe("save-file dialog default name", () => {
  it("suggests a name that does not overwrite an existing file", async () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-downloads-"));
  ...
  it("keeps the extension on the suggestion", async () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-downloads-ext-"));
    const source = path.join(botHome, "workspaces", "bot", "report.docx");
    fs.writeFileSync(path.join(downloads, "report.docx"), "");
...
describe("attachment download destination", () => {
  it("uses Chromium-style suffixes without replacing an existing download", () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-attachment-downloads-"));
    try {
      assert.equal(
  ...
  it("keeps renderer-supplied directories outside the destination", () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "agentbot-attachment-basename-"));
    try {
      assert.equal(
