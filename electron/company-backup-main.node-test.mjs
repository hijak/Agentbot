...
test("scheduled native transfer receives fresh client state and disable cancels its own transfer", async () => {
  ...
  f.invoke("client-state", { requestId: request.requestId, clientState: { "agentbot-drafts": "latest synthetic draft" } });
  const { input, signal } = await started.promise;
  assert.equal(input.clientState["agentbot-drafts"], "latest synthetic draft");
...
for (const [name, mutate] of [
  ...
  test(`client-state receipt is rejected after ${name}`, async () => {
    ...
    f.invoke("client-state", { requestId: request.requestId, clientState: { "agentbot-drafts": "fixture" } });
...
test("client-state receipts are one-shot and unsolicited or other-frame replies cannot settle them", async () => {
  ...
  f.invoke("client-state", { requestId: request.requestId, clientState: { "agentbot-skin": "fresh fixture skin" } });
  assert.equal((await collecting)["agentbot-skin"], "fresh fixture skin");
  assert.equal(f.context.companyBackupClientStateRequest, null);
  f.invoke("client-state", { requestId: request.requestId, clientState: { "agentbot-skin": "ignored duplicate" } });
});

for (const value of [{ unavailable: true }, { clientState: [] }, { clientState: { "agentbot-drafts": 42 } }, { clientState: { "agentbot-drafts": "x".repeat(2 * 1024 * 1024) } }]) {
