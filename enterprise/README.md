...
Set `AGENTBOT_LICENSE_KEY` on the server. The key is `omb1.<claims>.<signature>`:
...
{ "edition": "oss", "features": [], "notice": "AGENTBOT_LICENSE_KEY expired on 2027-09-02; renew it to keep enterprise features" }
```
...
`~/.config/agentbot-enterprise/signing-key.json`). Rotate by generating a
...
Operators opt a workspace into this protocol using `AGENTBOT_ADMIN_URL`,
`AGENTBOT_ADMIN_WORKSPACE` and `AGENTBOT_PUBLIC_URL`; a valid `admin` entitlement is
...
portal sessions with `AGENTBOT_ADMIN_MEMBERSHIP=portal`; this never exempts ordinary
email or pairing sessions. See the [protocol and isolated verification recipe](../docs/verification/hosted-workspaces.md).

## White-label (`whitelabel`)

Put a `brand.json` in the server's data dir (`AGENTBOT_DATA_DIR`, the `/data`
volume in Docker) or point `AGENTBOT_BRAND_FILE` at one:
