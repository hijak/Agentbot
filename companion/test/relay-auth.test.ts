...
it("adds the private capability only after authenticating and authorizing, never from client headers", async () => {
  ...
  const call = async (path: string, token = "paired", method = "POST") => {
    const res = await fetch(`http://127.0.0.1:${port}${path}`, { method, headers: {
      ...
      "x-agentbot-companion-auth": "forged",
      "x-agentbot-desktop-owner": "forged-owner",
      "x-agentbot-companion-device": "forged-phone",
    } });
    await res.text();
    return res.status;
  };
  ...
  expect(received[0]["x-agentbot-companion-auth"]).toBe(capability);
  expect(received[0]["x-agentbot-companion-device"]).toBe("real-phone");
  expect(received[0]["x-agentbot-desktop-owner"]).toBeUndefined();
  expect(received[0].authorization).toBeUndefined();
  await call("/api/health", "unpaired", "GET");
  expect(received[1]["x-agentbot-companion-auth"]).toBeUndefined();
});
