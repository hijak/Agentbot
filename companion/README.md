...
Agentbot's harness listens on `127.0.0.1` and nothing else, which is the
...
| Environment | Default | |
|---|---|---|
| `AGENTBOT_PORT` | `8799` | where the harness is |
| `AGENTBOT_WEBHOOK_PORT` | `AGENTBOT_PORT` + 1 | the harness's webhook receiver — refused, not used |
| `AGENTBOT_COMPANION_PORT` | `8810` | where devices connect |
| `AGENTBOT_CONTROL_PORT` | `8811` | the pairing page, loopback only |
| `AGENTBOT_COMPANION_DIR` | `~/.agentbot-companion` | paired devices live here |
| `AGENTBOT_COMPANION_NAME` | your name, from the harness | what the phone calls this computer |

`AGENTBOT_COMPANION_NAME` overrides a name the sidecar otherwise asks the harness
for at startup — the profile from onboarding, as *"Ada's computer"*. It falls
back to `Agentbot` when the harness is not up or has no profile. Read once
...
it (`AGENTBOT_WEBHOOK_PORT`). The companion refuses to start on either and says
