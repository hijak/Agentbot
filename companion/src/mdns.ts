 ...
 * record here:
 *
 * - **Shared records (§10.2).** The PTR of `_agentbot._tcp.local` is
...
export interface ServiceInfo {
  ...
  type: string;
  port: number;
  /** the name our A records claim, e.g. "agentbot-1a2b3c4d.local" */
  host: string;
  addresses: string[];
...
export function dnsLabel(text: string, fallback = "Agentbot"): string {
  let label = text
