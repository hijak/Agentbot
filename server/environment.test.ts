...
afterEach(() => {
  ...
  delete process.env.AGENTBOT_APP_VERSION;
  delete process.env.AGENTBOT_ENVIRONMENT_LABEL;
});

describe("environment identity", () => {
  it("creates the id once, owner-only, and keeps it across restarts", () => {
    const dir = mkdtempSync(join(tmpdir(), "agentbot-env-"));
    dirs.push(dir);
  ...
  it("rejects UUID-shaped garbage and an unreadable identity path", () => {
    const malformedDir = mkdtempSync(join(tmpdir(), "agentbot-env-"));
    ...
    const unreadableDir = mkdtempSync(join(tmpdir(), "agentbot-env-"));
  ...
  it("does not create the data directory merely by importing computer providers", async () => {
    const parent = mkdtempSync(join(tmpdir(), "agentbot-provider-import-"));
    dirs.push(parent);
  ...
  it("describes the server for clients without leaking anything secret", () => {
    process.env.AGENTBOT_APP_VERSION = "0.1.99";
    process.env.AGENTBOT_ENVIRONMENT_LABEL = "cab mini";
