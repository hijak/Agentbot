...
const liveToken = async (threadId: string): Promise<Record<string, string>> => {
  await expect.poll(() => dumpOf(threadId)?.mcpConfig?.mcpServers?.agents?.env?.AGENTBOT_COMMS_TOKEN, { timeout: 15_000 }).toBeTruthy();
  return { authorization: `Bearer ${dumpOf(threadId)!.mcpConfig.mcpServers.agents.env.AGENTBOT_COMMS_TOKEN}` };
};
...
const handoffs = (): any[] => JSON.parse(readFileSync(join(home, ".agentbot", "room-handoffs.json"), "utf8"));
...
beforeAll(async () => {
  ...
  home = mkdtempSync(join(tmpdir(), "agentbot-thread-aware-"));
  ...
  writeFileSync(gated, [
    ...
    "    for (const server of Object.values(servers)) thread ??= server?.env?.AGENTBOT_THREAD_ID ?? null;",
    "  } catch {}",
    "}",
