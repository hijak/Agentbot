...
const EXCLUDES = `# Agentbot checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "Agentbot Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@agentbot.local";
  env.GIT_COMMITTER_NAME = "Agentbot Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@agentbot.local";
  return env;
}
