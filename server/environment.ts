...
// /.well-known/agentbot/environment so a saved connection can check it is
 ...
 * an image sets AGENTBOT_APP_VERSION at build time. */
export function serverVersion(): string {
  const fromEnv = process.env.AGENTBOT_APP_VERSION?.trim();
...
export function environmentDescriptor(input: { environmentId: string; desktopManaged: boolean; emailSignIn?: boolean; sharedComputers?: boolean }): EnvironmentDescriptor {
  return {
    ...
    label: process.env.AGENTBOT_ENVIRONMENT_LABEL?.trim() || hostname(),
