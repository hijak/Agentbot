...
import { openMausStatusSystemPrompt, readOpenMausStatus } from "./agentbot-status-capsule.ts";
...
function cachePath(capsule: TestCapsule): string {
  ...
  roots.push(root);
  const parent = join(root, "agentbot");
...
posixOnly("readOpenMausStatus", () => {
  it("projects only fresh normalized two-VM capability data", () => {
    ...
    // Cross-language receipt produced by scripts/aos_agentbot_status.py
  ...
  it("rejects a non-0700 parent and a symlinked parent", () => {
    ...
    roots.push(linkRoot);
    const linkedParent = join(linkRoot, "agentbot");
