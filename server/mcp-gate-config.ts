 ...
 * bot servers directly as before. `AGENTBOT_MCP_RESULT_BUDGET=0` is the escape
...
export function resultBudget(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.AGENTBOT_MCP_RESULT_BUDGET;
...
export function gateServer(input: {
  ...
  return {
    ...
    env: {
      ...input.nodeEnv,
      AGENTBOT_GATE_NAME: name,
      AGENTBOT_GATE_UPSTREAM: JSON.stringify({ command: spec.command, args: spec.args ?? [], env: spec.env ?? {} }),
      AGENTBOT_GATE_SPILL_DIR: spillDir(input.threadId),
      AGENTBOT_GATE_BUDGET: String(budget),
    },
  };
}
