...
beforeAll(async () => {
  api = createServer(async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/api/v3.1/tool_router/session/trs_legacy") {
      ...
      return res.end(JSON.stringify({
        ...
        config: { user_id: "agentbot_legacy" },
      }));
    }
  ...
  process.env.AGENTBOT_COMPOSIO_API = base;
  process.env.AGENTBOT_COMPOSIO_TOOLKITS_API = `${origin}/api/v3`;
});

afterAll(async () => {
  setManagedBrokerAccess(null);
  delete process.env.AGENTBOT_COMPOSIO_API;
  delete process.env.AGENTBOT_COMPOSIO_TOOLKITS_API;
  await new Promise<void>((resolve) => api.close(() => resolve()));
});

describe.sequential("Composio Sessions", () => {
  it("rejects broker URL components and invalid tokens from the environment", () => {
    process.env.AGENTBOT_COMPOSIO_BROKER_TOKEN = "a".repeat(64);
    try {
      for (const url of [
        ...
        process.env.AGENTBOT_COMPOSIO_BROKER_URL = url;
        expect(() => connectionMode({})).toThrow(/must not include/);
      }
      process.env.AGENTBOT_COMPOSIO_BROKER_URL = "http://[::1]:3210/root/";
      expect(connectionMode({})).toBe("managed");
      process.env.AGENTBOT_COMPOSIO_BROKER_TOKEN = "short";
      expect(() => connectionMode({})).toThrow(/token is invalid/);
    } finally {
      delete process.env.AGENTBOT_COMPOSIO_BROKER_URL;
      delete process.env.AGENTBOT_COMPOSIO_BROKER_TOKEN;
    }
  });
  ...
  it("ignores credential sync without access and clears only on explicit null", () => {
    const messageType = "agentbot:managed-composio";
    setManagedBrokerAccess({ url: "http://127.0.0.1:3210/", token: "a".repeat(64) });
  ...
  it("drops a managed MCP session when routing switches to a user project", async () => {
    ...
    try {
      ...
      const project = await relayMcp(
        { composio: { apiKey: "ak_test", userId: "agentbot_existing", sessionId: "trs_test" } },
  ...
  it("creates one stable per-installation session and reuses it", async () => {
    const created = await prepareProjectSession("ak_test", { userId: "agentbot_existing" });
    expect(created).toEqual({
      apiKey: "ak_test",
      userId: "agentbot_existing",
      sessionId: "trs_test",
    });
  ...
  it("names the project's own auth configs at creation and rebuilds a Session that predates them", async () => {
    ...
    sessionAuthConfigs = {};
    try {
      const current = { apiKey: "ak_test", userId: "agentbot_existing", sessionId: "trs_test" };
      const before = calls.length;
      ...
      expect(creates[0].body).toMatchObject({ user_id: "agentbot_existing", auth_configs: { twitter: "ac_twitter" } });
  ...
  it("rebuilds the Session and retries when a toolkit needs the project's own auth config", async () => {
    ...
    try {
      ...
      expect(cfg.composio).toMatchObject({ userId: "agentbot_existing", sessionId: "trs_test" });
    } finally {
      customAuthConfigs = [];
      sessionAuthConfigs = {};
    }
  ...
  it("does not offer Twitter through the official managed broker without an owned OAuth app", async () => {
    setManagedBrokerAccess({
      url: "https://broker.agentbot.test",
      token: "a".repeat(64),
    });
    try {
  ...
  it("mounts the Session MCP endpoint with the project key header", async () => {
    ...
    expect(integration).toMatchObject({
      ...
      env: {
        ...
        AGENTBOT_CONNECTOR_UPSTREAM_HEADERS: JSON.stringify({ authorization: "Bearer secret" }),
        AGENTBOT_HARNESS_URL: "http://127.0.0.1:8799",
        AGENTBOT_CONNECTOR_TOKEN: "secret",
