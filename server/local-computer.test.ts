  ...
  return {
    ...
    mcp: {
      ...
      env: {
        ...
        CUA_DRIVER_HOST_BUNDLE_ID: "com.agentbot.app",
...
describe("local computer descriptor contract", () => {
  it("wraps only the transport while preserving descriptor identity and original driver environment", () => {
    ...
    expect(gated.env).toEqual({ ...connection.env, ELECTRON_RUN_AS_NODE: "1", AGENTBOT_CUA_COMMAND: connection.command, AGENTBOT_CUA_ARGS: JSON.stringify(connection.args), AGENTBOT_CONTROL_URL: "http://127.0.0.1:1234/control", AGENTBOT_CONTROL_TOKEN: "fixture-token" });
    expect(gated.args.join(" ")).not.toContain("fixture-token");
    expect(connection.env).not.toHaveProperty("AGENTBOT_CONTROL_TOKEN");
  });
...
function privateUserData(name: string) {
  ...
  const root = mkdtempSync(join(base, "agentbot-local-computer-"));
