...
const NAME = process.env.AGENTBOT_GATE_NAME || "mcp";
const SPILL_DIR = process.env.AGENTBOT_GATE_SPILL_DIR || "";
const BUDGET = Number(process.env.AGENTBOT_GATE_BUDGET) > 0 ? Number(process.env.AGENTBOT_GATE_BUDGET) : DEFAULT_RESULT_BUDGET;
...
const SPILL_HINT = process.env.AGENTBOT_GATE_SPILL_HINT === "1";

/** The gate's own settings never reach the upstream server's environment. */
const GATE_ENV_KEYS = ["AGENTBOT_GATE_NAME", "AGENTBOT_GATE_SPILL_DIR", "AGENTBOT_GATE_BUDGET", "AGENTBOT_GATE_UPSTREAM", "AGENTBOT_GATE_SPILL_HINT"];
...
function upstreamSpec(): Upstream {
  let parsed: unknown;
  try {
    parsed = JSON.parse(process.env.AGENTBOT_GATE_UPSTREAM ?? "");
  } catch {
    fail("AGENTBOT_GATE_UPSTREAM is not valid JSON");
  }
  const spec = parsed as Upstream | null;
  if (!spec || typeof spec !== "object" || typeof spec.command !== "string" || !spec.command) {
    fail("AGENTBOT_GATE_UPSTREAM needs a command");
  }
  return spec;
}
