...
function apiBase() {
  return (process.env.AGENTBOT_COMPOSIO_API ?? `${DEFAULT_BACKEND_ORIGIN}/api/v3.1`).replace(/\/$/, "");
}

function toolkitBase() {
  return (process.env.AGENTBOT_COMPOSIO_TOOLKITS_API ?? `${DEFAULT_BACKEND_ORIGIN}/api/v3`).replace(/\/$/, "");
}
...
export function applyManagedBrokerMessage(message: unknown): boolean {
  ...
  if (
    !parsed.success ||
    parsed.data.type !== "agentbot:managed-composio" ||
...
function brokerAccess(): { url: string; token: string } | null {
  ...
  const url = process.env.AGENTBOT_COMPOSIO_BROKER_URL?.trim();
  const token = process.env.AGENTBOT_COMPOSIO_BROKER_TOKEN?.trim();
...
/** Three answers, not two. The desktop shell sets AGENTBOT_CREDENTIAL_STORE to
...
export function connectorAvailability(
  cfg: AppConfig,
  storeState: string | undefined = process.env.AGENTBOT_CREDENTIAL_STORE,
...
export async function prepareProjectSession(
  ...
  if (trimmed === current?.apiKey && current.sessionId) {
    ...
    if (
      ...
      return {
        apiKey: trimmed,
        userId: existing.config?.user_id ?? current.userId ?? `agentbot_${randomUUID()}`,
  ...
  const userId = priorUserId ?? `agentbot_${randomUUID()}`;
...
export async function mcpIntegration(
  ...
  return {
    ...
    env: {
      ...
      AGENTBOT_CONNECTOR_UPSTREAM_URL: `${context.harnessUrl}/api/internal/connectors/mcp`,
      AGENTBOT_CONNECTOR_UPSTREAM_HEADERS: JSON.stringify({ authorization: `Bearer ${context.commsToken}` }),
      AGENTBOT_HARNESS_URL: context.harnessUrl,
      ...
      AGENTBOT_CONNECTOR_TOKEN: context.commsToken,
      AGENTBOT_BOT_ID: context.botId,
      AGENTBOT_THREAD_ID: context.threadId,
    },
  };
}
