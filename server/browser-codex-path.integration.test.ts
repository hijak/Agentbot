...
describe.skipIf(process.platform === "win32")("Codex browser turns with a minimal GUI PATH", () => {
  ...
  beforeAll(async () => {
    home = mkdtempSync(join(tmpdir(), "agentbot-browser-codex-path-"));
    ...
    child = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
      cwd: join(SERVER_DIR, ".."),
      env: {
        PATH: FINDER_PATH, HOME: home, AGENTBOT_DATA_DIR: data, AGENTBOT_PORT: String(port),
        AGENTBOT_AGENT_BROWSER_PATH: browser, VITEST: "1",
      },
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.stdout!.on("data", (chunk) => {
        output += chunk;
        if (output.includes(`agentbot server on ${base}`)) { clearTimeout(timer); resolve(); }
      });
    });
  ...
  it.each(["default", "absolute"])("keeps the browser mounted and completes a turn with the %s Codex CLI", async (instanceId) => {
    ...
    expect(dump.env.AGENTBOT_BROWSER_TOKEN).toBeTruthy();
