...
// DATA_DIR (~/.agentbot) never touches the real one. os.homedir()
...
const home = mkdtempSync(join(tmpdir(), "agentbot-test-home-"));
process.env.HOME = home;
process.env.USERPROFILE = home;
// AGENTBOT_DATA_DIR is an intentional production override, but tests must never
// let it escape the throwaway home they are about to delete.
delete process.env.AGENTBOT_DATA_DIR;
...
process.env.AGENTBOT_COMPANION_DIR = join(home, ".agentbot-companion");
