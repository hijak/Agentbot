...
const gate = (c: any) => fetch(c.env.AGENTBOT_CONTROL_URL, { headers: { authorization: `Bearer ${c.env.AGENTBOT_CONTROL_TOKEN}` } });

beforeAll(async () => {
  fixtureHome = mkdtempSync(join(tmpdir(), "agentbot-group-vm-"));
  ...
  child = spawn(process.execPath, ["--import", pathToFileURL(join(ROOT, "server/testing/group-local-vm-hooks.mjs")).href, join(ROOT, "server/index.ts")], {
    cwd: ROOT, env: {
      ...
      HOME: fixtureHome, USERPROFILE: fixtureHome, AGENTBOT_DATA_DIR: data,
      ...
      AGENTBOT_PORT: String(port), AGENTBOT_WEBHOOK_PORT: String(port + 1), AGENTBOT_STATIC_DIR: ui, AGENTBOT_TEST_VM_STATE: stateFile,
      AGENTBOT_BOX_API: `http://127.0.0.1:${boxPort}`,
...
describe("Group Local VM ownership on the real isolated server", () => {
  it.each(["wake", "removed", "missing-auto"])("chat selection starts or provisions a configured cloud computer (%s) only after selecting it", async state => {
    ...
    try {
      ...
      const token = before.mcpConfig.mcpServers.agents.env.AGENTBOT_COMMS_TOKEN;
  ...
  it.each(["stop", "failure", "off", "manual-selection", "new-request"])("does not continue a computer selection after %s", async failure => {
    ...
    try {
      ...
      const token = sent.mcpConfig.mcpServers.agents.env.AGENTBOT_COMMS_TOKEN;
  ...
  it("revokes the previous member and rejects cross-bot control after a shared desktop handoff", async () => {
    ...
    expect((await gate(first)).status).toBe(401);
    const impersonation = await fetch(second.env.AGENTBOT_CONTROL_URL.replace(bots[1].id, bots[0].id), {
      headers: { authorization: `Bearer ${second.env.AGENTBOT_CONTROL_TOKEN}` },
    });
