...
export const VPS_MANAGED_LABEL = "com.agentbot.vps";
export const VPS_CONTAINER_LABEL = "com.agentbot.container";
export const VPS_ENVIRONMENT_LABEL = "com.agentbot.environment";
export const VPS_VIEWER_LABEL = "com.agentbot.vps-viewer";
export const VPS_CONTAINER_PREFIX = "agentbot-vps";
...
const MANAGED_VPS_CONTAINER_NAME = /^agentbot-vps-[a-z0-9]{1,12}-[a-f0-9]{12}$/;
const IMAGE_ID = /^sha256:[a-f0-9]{64}$/i;
const PIDS_LIMIT = 512;
const SCREENSHOT_PATH = "/tmp/agentbot-vps-preview.png";
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned Agentbot Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No Agentbot container exists for this bot on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted Agentbot image";
  if (!status.managed) return "The VPS container name is occupied by a container Agentbot did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing Agentbot safety limits";
  if (status.container === "stopped") return "The Agentbot VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        // never docker-rm a container Agentbot did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container Agentbot did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
