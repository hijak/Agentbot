...
// into first-class Agentbot chat cards. The agent never authors an auth
...
const UPSTREAM = process.env.AGENTBOT_CONNECTOR_UPSTREAM_URL ?? "";
...
const TOKEN = process.env.AGENTBOT_CONNECTOR_TOKEN ?? process.env.AGENTBOT_COMMS_TOKEN ?? "";
...
function parsedHeaders(): Record<string, string> {
  try {
    const value: unknown = JSON.parse(process.env.AGENTBOT_CONNECTOR_UPSTREAM_HEADERS ?? "{}");
...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (requests.length) {
      ...
      send(textResult(
        id,
        `Agentbot showed the user a secure connection card for ${labels}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
    if (/WAIT_FOR_CONNECTIONS$/i.test(name)) {
      send(textResult(id, "Agentbot is handling connection completion and will continue the task automatically."));
      return;
    }
  }
  try {
