...
//
// Agentbot owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/agentbot/cua-local-vm";
export const IMAGE_LAYER_VERSION = "5";
export const IMAGE_LAYER_LABEL = "com.agentbot.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "agentbot-computer";
export const MANAGED_LABEL = "com.agentbot.local-vm";
export const DRIVER_LABEL = "com.agentbot.cua-driver";
export const BASE_IMAGE_LABEL = "com.agentbot.cua-base";
export const WORKSPACE_LABEL = "com.agentbot.workspace";
export const TARGET_LABEL = "com.agentbot.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/agentbot-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/agentbot/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-agentbot-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-agentbot-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-agentbot-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-agentbot-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-agentbot-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:agentbot-cua-driver]' \\
      'command=/usr/local/bin/start-agentbot-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by Agentbot; recreate it";
 ...
 * versions. An older Agentbot container must stay removable (and eligible
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared Agentbot derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/agentbot-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "agentbot-cua-image-"));
  try {
...
export async function containerComputerAction(
  ...
  if (action === "remove" && !before.managed) {
    throw Object.assign(
      new Error(
        `The existing container named ${target.containerName} was not created by Agentbot; remove it manually in ${runtime}`,
      ),
      { status: 409 },
    );
  }
...
export async function containerComputerFrame(
  ...
  try {
    const screenshot = "/tmp/agentbot-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
export function containerComputerMcp(
  ...
  return {
    ...
    env: {
      ...
      ...(control ? { AGENTBOT_CONTROL_URL: control.url, AGENTBOT_CONTROL_TOKEN: control.token } : {}),
    },
  };
}
