// `agentbot` on the command line: run the server anywhere and pair devices
// to it. One implementation for three homes — `npx agentbot` (the npm
...
//
//   agentbot setup [--data-dir ~/.agentbot]
//   agentbot start [serve options]
//   agentbot serve [--port 8799] [--data-dir ~/.agentbot] [--label "cab mini"]
//                     [--public-url https://host] [--tailscale | --domain HOST] [--no-pair]
//   agentbot pair  [--label "My MacBook"] [--client] [--public-url https://host]
//   agentbot sessions [revoke <id>]
//   agentbot status
//
...
//
// This module only exports; agentbot.ts is the entry that runs main(), so
...
export interface CliOptions {
  command: "setup" | "start" | "serve" | "pair" | "sessions" | "status" | "access" | "service" | "browser" | "fleet" | "help";
  port: number;
  dataDir: string;
  label?: string;
  publicUrl?: string;
...
const COMMANDS = ["setup", "start", "serve", "pair", "sessions", "status", "access", "service", "browser", "fleet", "help", "--help", "-h"];
...
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env): CliOptions | { error: string } {
  ...
  const options: CliOptions = {
    ...
    port: Number(env.AGENTBOT_PORT || 8799),
    dataDir: env.AGENTBOT_DATA_DIR || join(homedir(), ".agentbot"),
    tailscale: false,
    client: false,
    pair: true,
    withDeps: false,
  ...
  if (options.domain && (options.tailscale || options.publicUrl)) return { error: "--domain already gives the server its address; drop --tailscale and --public-url" };
  if (options.local && (options.tailscale || options.publicUrl)) return { error: "--local cannot be combined with a remote-access option" };
...
export const USAGE = `agentbot — your team of AI bots, ready in a few steps

  agentbot                         set up once, then open your workspace
  agentbot setup [--data-dir DIR]
  agentbot start [the same options as serve]
  agentbot serve [--port 8799] [--data-dir DIR] [--label NAME]
                    [--public-url https://host] [--tailscale | --domain HOST] [--no-pair]
  agentbot pair  [--label NAME] [--client] [--phone ios|android]
                    [--public-url https://host]
  agentbot sessions [revoke ID]
  agentbot status
  agentbot access list | add EMAIL [--chat-only] | remove EMAIL
  agentbot service install [--domain HOST | --tailscale] [--port N] [--data-dir DIR] | uninstall
  agentbot browser install [--with-deps] | status
  agentbot fleet init --domain HOST [--operator USER] | create NAME --admin EMAIL [--member EMAIL] [--brand FILE]
...
start   same as agentbot: use your saved settings and open the workspace
...
service keep the server running across reboots: writes a systemd unit
        ...
        permanently first (npm install -g agentbot).
...
Install once with \`npm install -g agentbot\`, then type \`agentbot\`.
Or run without a global install: \`npx agentbot\`. Node 24+ is required.
`;
...
async function serverUp(port: number, pid?: number): Promise<boolean> {
  try {
    ...
    return status === 200 && body?.app === "agentbot" && (pid === undefined || body.pid === pid);
  } catch {
    return false;
  }
}
...
export async function isWorkspaceRunning(options: CliOptions): Promise<boolean> {
  try {
    ...
    if (status !== 200 || body?.app !== "agentbot") return false;
    const expected = readFileSync(join(options.dataDir, "environment-id"), "utf8").trim();
    const descriptor = await api(options.port, "/.well-known/agentbot/environment");
...
export async function verifyPhoneEndpoint(port: number, origin: string): Promise<boolean> {
  ...
  try {
    const local = await api(port, "/.well-known/agentbot/environment");
    const remote = await fetch(`${origin}/.well-known/agentbot/environment`, { signal: AbortSignal.timeout(5000), redirect: "error" });
...
export function applyStartupPreferences(options: CliOptions, saved: AppConfig["cliStartup"]): CliOptions {
  if (options.local) return { ...options, tailscale: false, publicUrl: undefined, phone: undefined };
  if (!saved || options.tailscale || options.publicUrl) return options;
  if (saved.access === "public-url" && (!saved.publicUrl || !normalizePhoneOrigin(saved.publicUrl))) {
    throw new Error("The saved phone address is not a valid HTTPS origin. Run agentbot setup to correct it, or agentbot --local to start only on this computer.");
  }
  return {
    ...options,
    tailscale: saved.access === "tailscale",
...
function startupPreferences(options: CliOptions): NonNullable<AppConfig["cliStartup"]> {
  const access = options.local ? "local" : options.tailscale ? "tailscale" : options.publicUrl ? "public-url" : "local";
...
async function showPhonePairing(options: CliOptions, origin: string | undefined, log: (line: string) => void): Promise<boolean> {
  ...
  if (!ready) {
    ...
    log("Check the HTTPS connection, then run agentbot pair again with the same --data-dir and --port.");
    return false;
  }
 ...
 * the iOS app read. `inviteUrl` is the agentbot:// scheme the native
...
export function pairingBlock(input: {
  ...
  if (target) {
    ...
    lines.push("");
    if (scanInvite) {
      lines.push(`Scan that in the Agentbot app. For a browser instead, open the web`);
      ...
      lines.push(`AGENTBOT_PUBLIC_URL, or open the web address above and type the code.`);
    } else if (input.inviteUrl) {
      lines.push(`Scan that with Camera for the browser, or paste the phone-app link`);
      lines.push(`above into the Agentbot app.`);
    }
  }
  return lines.join("\n");
}
...
async function mintPairing(port: number, options: { label?: string; client?: boolean; publicUrl?: string; phone?: "ios" | "android" }): Promise<string> {
  ...
  // AGENTBOT_PUBLIC_URL returns a credential and no invite, and gating on the
  ...
  const invite = typeof body.credential === "string" && address
    ? `agentbot://pair?address=${encodeURIComponent(address)}&token=${encodeURIComponent(body.credential)}${typeof body.serverName === "string" ? `&name=${encodeURIComponent(body.serverName)}` : ""}`
...
export async function runPair(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no Agentbot server on http://127.0.0.1:${options.port}; start one with \`agentbot serve\` or set AGENTBOT_PORT`);
    return 1;
  }
  if (process.stdin.isTTY && process.stdout.isTTY && !options.label && !options.client) {
    ...
    if (!origin || !normalizePhoneOrigin(origin)) {
      ...
      console.log("Stop the server, run agentbot setup and choose phone access, then start agentbot again.");
      return 1;
    }
...
export async function runSessions(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no Agentbot server on http://127.0.0.1:${options.port}`);
    return 1;
  }
  if (options.revoke) {
  ...
  if (!sessions.length) {
    console.log("no paired devices yet: run `agentbot pair`");
    return 0;
  }
...
export function formatSessions(sessions: Array<{ id: string; label: string; scopes: string[]; lastSeenAt: number; expiresAt: number }>, now = Date.now()): string {
  ...
  return [line(head), ...rows.map(line), "", "revoke one with: agentbot sessions revoke <id>"].join("\n");
}

export async function runStatus(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  let code = 0;
  try {
    const res = await fetch(`http://127.0.0.1:${options.port}/.well-known/agentbot/environment`);
    const body: any = await res.json();
    io.log(options.json ? JSON.stringify(body, null, 2) : `${body.label} · Agentbot ${body.version} on ${body.platform} · id ${body.environmentId}`);
  } catch {
    io.error(`no Agentbot server on http://127.0.0.1:${options.port}`);
    code = 1;
  }
  return code;
}
 ...
 * needs no restart. Environment variables (AGENTBOT_SIGNIN_EMAILS) win when set.
...
export async function runAccess(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  const overridden = process.env.AGENTBOT_SIGNIN_EMAILS !== undefined || process.env.AGENTBOT_SIGNIN_MEMBER_EMAILS !== undefined;
  ...
  if (options.accessAction === "list") {
    if (!admins.length && !members.length) {
      io.log("nobody can sign in with an email yet; pairing codes only. Add someone with: agentbot access add you@example.com");
      return 0;
    }
    ...
    if (overridden) io.log("(AGENTBOT_SIGNIN_EMAILS / AGENTBOT_SIGNIN_MEMBER_EMAILS are set in the environment and win over this list while the server runs)");
    return 0;
  }
  ...
  if (options.accessAction === "remove") {
    ...
    io.log(`${entry} can no longer sign in (existing sessions stay until they expire or are revoked with \`agentbot sessions revoke\`)`);
    return 0;
  }
  ...
  if (overridden) io.log("note: AGENTBOT_SIGNIN_EMAILS / AGENTBOT_SIGNIN_MEMBER_EMAILS are set in the environment and win over this list while the server runs");
  return 0;
}
...
export async function runBrowser(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  if (options.browserAction === "status") {
    ...
    if (status.kind !== "ready" && status.installable) io.log("install it with:  agentbot browser install");
  ...
  try {
    ...
    if (process.platform === "linux" && !options.withDeps) io.error("on Linux, install Chrome's system libraries with `sudo agentbot browser install --with-deps`, then retry `agentbot browser install` as the user running serve");
    return 1;
  }
  ...
  if (process.platform === "linux" && options.withDeps) io.log("if serve runs as another user, run `agentbot browser install` from that user's login shell too");
  return 0;
}
...
export async function runServe(options: CliOptions, log: (line: string) => void = console.log): Promise<number> {
  ...
  if (await serverUp(options.port)) {
    console.error(`something already answers on http://127.0.0.1:${options.port}; use \`agentbot pair\` against it, or --port for a second server`);
    return 1;
  }
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    AGENTBOT_DATA_DIR: options.dataDir,
    AGENTBOT_PORT: String(options.port),
    AGENTBOT_WEBHOOK_PORT: process.env.AGENTBOT_WEBHOOK_PORT || String(options.port + 1),
  };
  if (options.local) delete env.AGENTBOT_PUBLIC_URL;
  if (entry.staticDir) env.AGENTBOT_STATIC_DIR = entry.staticDir;
  if (entry.skillsDir && !process.env.AGENTBOT_SKILLS_DIR) env.AGENTBOT_SKILLS_DIR = entry.skillsDir;
  if (options.label && !process.env.AGENTBOT_ENVIRONMENT_LABEL) env.AGENTBOT_ENVIRONMENT_LABEL = options.label;
  ...
  try {
    ...
    if (startupCancelled) throw new SetupCancelled();
    if (publicUrl) env.AGENTBOT_PUBLIC_URL = publicUrl;
  ...
  const stop = () => {
    stopping ??= (async () => {
      // The edge stops accepting before the server it forwards to goes away.
  ...
  try {
    ...
    if (exited !== null) {
      if (exited !== 0) log(`Agentbot could not start.${logPath ? ` Details: ${logPath}` : " See the output above."}`);
      return exited;
    }
    if (stopping) return await childExit;
    if (!(await serverUp(options.port, child.pid))) {
      console.error(`Agentbot did not become ready within a minute.${logPath ? ` Details: ${logPath}` : " See its output above."}`);
      await stop();
      return 1;
    }
    ...
    if (options.domain && caddyBinary) {
      try {
        caddy = await startCaddy({ binary: caddyBinary, dataDir: options.dataDir, domain: options.domain, appPort: options.port, webhookPort: Number(env.AGENTBOT_WEBHOOK_PORT), log });
    ...
    log("");
    log(`Agentbot is running on http://127.0.0.1:${options.port}${publicUrl ? `, reachable at ${publicUrl}` : ""}`);
    if (options.guided) {
    ...
    if (options.pair && options.phone) {
      ...
      log("");
      log("another device later:  agentbot pair --label \"Kitchen iPad\"");
    }
    log(options.guided ? "\nKeep this terminal open while using your bots. Ctrl+C stops the server, not your saved work." : "stop with Ctrl+C");
    if (options.guided) log("Next time: agentbot · Change AI or phone setup: agentbot setup · Pair another phone: agentbot pair");
...
export async function runOnboardingCommand(
  ...
  if (options.command === "setup" && !interactive) {
    io.error("Setup needs an interactive terminal. Run `npx agentbot setup` in a terminal, then use `npx agentbot serve` for unattended starts.");
    return 1;
  }
  process.env.AGENTBOT_DATA_DIR = options.dataDir;
  if (options.command !== "setup" && await (flow.running ?? isWorkspaceRunning)(options)) {
    if (options.local || options.tailscale || options.publicUrl) {
  ...
  try {
    if (options.command === "setup" || !(await isSetupComplete(options.dataDir))) {
      if (!interactive) {
        io.error("No completed setup was found. Run `npx agentbot setup` in an interactive terminal first, or use `npx agentbot serve` with an existing configuration.");
        return 1;
      }
      if (!(await runSetup({ dataDir: options.dataDir, port: options.port }))) {
        io.log("Setup cancelled. Run agentbot when you're ready.");
        return 130;
      }
    }
    ...
    if (options.command === "setup") {
      io.log("\nAll set. Start with: agentbot (or npx agentbot without a global install).");
      if (options.dataDir !== join(homedir(), ".agentbot") || options.port !== 8799) {
    ...
    if (!(error instanceof SetupCancelled)) throw error;
    io.log("\nSetup stopped. Any AI setup already saved is kept; no server was started. Run agentbot setup to continue.");
    return 130;
  }
}
    ...
    case "fleet":
      if (options.fleetAction === "agent") {
        ...
        await startFleetAgent({
          ...
          node: process.execPath,
          script: process.argv[1] ?? "",
          licenseKey: options.licenseKey ?? process.env.AGENTBOT_LICENSE_KEY,
        }, { log: (line) => console.log(line) });
