...
it("boots and reports an interrupted routine back to its source room", async () => {
  ...
  try {
    ...
    Object.assign(env, {
      ...
      AGENTBOT_PORT: new URL(url).port, AGENTBOT_WEBHOOK_PORT: String(Number(new URL(url).port) + 1),
    ...
    expect(runs[0]).toMatchObject({ status: "failed", error: "Agentbot restarted while this routine was running" });
