...
import {
  ...
  runControlOmb,
} from "../scripts/control-agentbot.ts";
import { installedChrome, UI_MUTATING } from "../scripts/testing/control-agentbot-ui.ts";
import { removeTempDir } from "./testing/cleanup.ts";

describe("control-agentbot command mapping", () => {
  ...
  it("keeps every ui verb off discovery: the launch handle is required, whatever the environment says", async () => {
    const env = { OPENMAUSBOT_URL: "http://127.0.0.1:19999", AGENTBOT_PORT: "19999" };
  ...
  it("refuses a handle whose launch has already been stopped", async () => {
    const dir = mkdtempSync(join(tmpdir(), "agentbot-ui-handle-"));
    try {
      const handle = join(dir, "ui.json");
      writeFileSync(handle, JSON.stringify({
        url: "http://127.0.0.1:19999", previewUrl: "http://127.0.0.1:5199/__threads.html", session: "agentbot-ui-19999",
  ...
  it("finds the newest Chrome for Testing that agent-browser install unpacked", () => {
    const dir = mkdtempSync(join(tmpdir(), "agentbot-ui-tools-"));
    try {
  ...
  it("runs directly under Node's strip-only TypeScript loader", () => {
    const result = spawnSync(process.execPath, [
      ...
      join(process.cwd(), "scripts", "control-agentbot.ts"),
      "help",
    ], { encoding: "utf8" });
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("control-agentbot");
  });
  ...
  it("rejects an available engine when the endpoint is not Agentbot", async () => {
    const callTool = vi.fn(async (name: string) => name === "get_system_health"
  ...
  it("refuses to mutate a silently discovered live app", async () => {
    await expect(runControlOmb(["new-bot", "--name", "Probe"], {
      ...
      message: "mutating commands require an explicit Agentbot instance",
    });
  });
  ...
  it("forwards pinned task IDs for sends, reads, waits, interrupts, and model changes", async () => {
    ...
    await expect(runControlOmb(["set-model", "--bot", "bot-1", "--instance", "claude", "--model", "model-b"], {
      callTool: callTool as any, env: {},
    })).rejects.toThrow("explicit Agentbot instance");
  });
});

describe("control-agentbot isolated verification loop", () => {
  it.each([
    "tcp://127.0.0.1:2375",
  ...
  it("launches, drives a real fake-engine turn, and removes only its test data", async () => {
    const parentEnv = {
      ...
      AGENTBOT_SKILLS_DIR: "/must/not/reach/the/fixture",
    ...
    try {
      ...
      expect(fixtureEnv).not.toHaveProperty("AGENTBOT_SKILLS_DIR");
