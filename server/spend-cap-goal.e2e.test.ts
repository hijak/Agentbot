...
describe("spend cap inside a team goal run", () => {
  ...
  beforeEach(async () => {
    layerDir = mkdtempSync(join(tmpdir(), "agentbot-fake-layer-goal-"));
    ...
    session = await launchVerificationServer({
      ...
      FAKE_CLAUDE_REPLIES: JSON.stringify([
        'The plan is ready.\n<agentbot-goal>{"status":"continue","next":"Worker","instruction":"Do the thing","detail":"Plan ready"}</agentbot-goal>',
      ]),
