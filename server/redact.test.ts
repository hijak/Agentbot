...
describe("redactSecrets", () => {
  it("masks the tokens in an ACP session/new, keeping the shape", () => {
        ...
        mcpServers: [
          {
            ...
            env: [
              { name: "AGENTBOT_BOT_ID", value: "bot-123" },
              { name: "AGENTBOT_COMMS_TOKEN", value: "s3cret-comms-token-value" },
            ],
          },
          {
            name: "computer",
    ...
    expect(out).toContain("AGENTBOT_COMMS_TOKEN");
...
describe("redactSecretsInText", () => {
  ...
  it("is idempotent for structurally identified credentials", () => {
    const input = {
      ...
      env: [{ name: "AGENTBOT_COMMS_TOKEN", value: "abcdefghijklmnop" }],
    };
