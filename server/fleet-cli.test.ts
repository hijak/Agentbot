...
const base: FleetInput = {
  ...
  node: "/usr/bin/node", script: "/usr/lib/node_modules/agentbot/cli.js", root: "/",
};
const registryFile = "/etc/agentbot/fleet.json";
const withRegistry = (workspaces = {}) => ({ [registryFile]: JSON.stringify({ ...emptyRegistry("agentada.cc"), workspaces }) });

describe("agentbot fleet", () => {
  it("updates only managed OpenRouter models and rejects missing, malformed or redirected config", async () => {
    ...
    const path = "/var/lib/agentbot/acme/.config/opencode/opencode.json";
    const cfgPath = "/var/lib/agentbot/acme/.agentbot/config.json";
    ...
    const initialFiles = { ...withRegistry({ acme: workspace }), [path]: JSON.stringify(config), [cfgPath]: '{"defaultModelSelection":{"instanceId":"claude","model":"keep"}}', "/etc/agentbot/instances/acme.env": "AGENTBOT_ADMIN_URL=https://admin.example.test\nAGENTBOT_ADMIN_WORKSPACE=acme\n" };
  ...
  it("prints the plan instead of acting when not root, and on --dry-run even as root", async () => {
    ...
    expect(out.join("\n")).toContain("cat > /etc/systemd/system/agentbot@.service <<'AGENTBOT_EOF'");
    expect(out.join("\n")).toContain("systemctl enable --now agentbot-fence.service");
    ...
    expect(dry.out.join("\n")).toContain("useradd --system --create-home --home-dir /var/lib/agentbot/acme");
  ...
  it("refuses an npx cache as the unit's script and asks for the domain", async () => {
    ...
    const bad = io();
    expect(await runFleetCommand({ ...base, action: "init", domain: "agentada.cc", script: "/root/.npm/_npx/abc/node_modules/agentbot/cli.js" }, bad.io, deps)).toBe(2);
  ...
  it("creates a workspace as root in order, and stops at the first failed step without repeating secrets", async () => {
    ...
    expect(calls.slice(0, 5)).toEqual([
      ...
      "run useradd --system --create-home --home-dir /var/lib/agentbot/acme --shell /usr/sbin/nologin --user-group omb-acme",
      `write ${registryFile} 600`,
      "mkdir /var/lib/agentbot/acme/.agentbot 700 as omb-acme",
      "write /var/lib/agentbot/acme/.agentbot/config.json 600 as omb-acme",
    ]);
    expect(JSON.parse(files.get("/var/lib/agentbot/acme/.agentbot/config.json")!)).toEqual({
      signIn: { admins: ["ada@example.test"], members: ["@acme.test"] }, anthropic: { key: "sk-ant-fixture" }, budgets: { monthlyUsd: 40 },
    });
    expect(files.get("/var/lib/agentbot/acme/.agentbot/brand.json")).toBe('{"name":"Acme"}');
    expect(files.get("/etc/agentbot/instances/acme.env")).toContain("AGENTBOT_LICENSE_KEY=omb1.k");
  ...
  it("edits a workspace's sign-in list in place, owned by the workspace, and lists what runs", async () => {
    const dataFile = "/var/lib/agentbot/acme/.agentbot/config.json";
  ...
  it("reinitializes templates without losing the operator, reservations, ports or fence", async () => {
    ...
    expect(ready.files.get("/etc/agentbot/fence.nft")).toBe(fenceRules(Object.values(workspaces)));
    expect(ready.files.get("/etc/systemd/system/agentbot-fleet.service")).toContain("--group maus");
    expect(ready.calls).toContain("run nft -f /etc/agentbot/fence.nft");
  ...
  it.each(["/var/lib/agentbot/acme", "/etc/agentbot/instances/acme.env", "/etc/caddy/agentbot.d/acme.caddy", "/etc/systemd/system/agentbot@acme.service.d"])("refuses residual customer path %s before taking action", async (path) => {
  ...
  it.each(["provisioning", "error", "retained"])("lists a %s reservation accurately and rejects duplicate creation before residual paths", async (status) => {
    const ready = machine({ root: true, files: { ...withRegistry({ acme: { slug: "acme", host: "acme.agentada.cc", port: 8810, webhookPort: 8811, status, createdAt: "" } }), "/etc/agentbot/instances/acme.env": "retained" } });
    const listed = io();
