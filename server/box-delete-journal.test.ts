...
describe("Box deletion journal", () => {
  ...
  beforeEach(() => {
    dataDir = mkdtempSync(join(tmpdir(), "agentbot-box-delete-journal-"));
  ...
  it("serializes independent processes without losing either prepared deletion", async () => {
    const concurrentDir = mkdtempSync(join(tmpdir(), "agentbot-box-delete-processes-"));
  ...
  it("recovers a complete lock left by an exited process", async () => {
    const staleDir = mkdtempSync(join(tmpdir(), "agentbot-box-delete-stale-lock-"));
