...
export function createBotPackageExport(input: {
  ...
  const id = portableKey(input.name, "agentbot-package", new Set());
  ...
  const definition: BotPackageDefinition = {
    ...
    release: "1.0.0",
    name: input.name,
    tagline: `A portable Agentbot setup with ${bots.length} ${bots.length === 1 ? "bot" : "bots"}.`,
    summary: "Exported from Agentbot. Review the roles, rooms, playbooks, connector requirements, and paused routines before sharing or publishing.",
    category: "Community",
    author: { name: input.authorName?.trim() || "Agentbot user" },
    license: "Unspecified",
