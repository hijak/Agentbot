...
export function fleetSocketPath(env: NodeJS.ProcessEnv = process.env): string {
  return env.AGENTBOT_FLEET_SOCKET?.trim() || "/run/agentbot/fleet.sock";
}
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (error: NodeJS.ErrnoException) => {
      reject(new Error(error.code === "ENOENT" || error.code === "ECONNREFUSED"
        ? "no fleet agent on this server: run `agentbot fleet init --domain … --operator <this user>` as root"
        : error.code === "EACCES"
          ? "this workspace's user may not open the fleet socket: re-run `agentbot fleet init` with --operator set to it"
