...
const EXCLUDED = new Set([
  ...
  ".agentbot-server-child", "environment-id", "sessions.json", "tunnel-account.json",
  "team-computers.json",
  "agentbot-server.lease", "box-create-requests.lock", "messages.db-wal", "messages.db-shm",
]);
...
function excluded(name: string): boolean {
  return EXCLUDED.has(name) || excludedWorkspaceAuthPath(name) || name.startsWith("agentbot-server.lease.") || name.startsWith("box-create-requests.lock.") || /^perm-[A-Za-z0-9_-]+\.sock$/.test(name);
}
...
export async function createWorkspaceBackup(dataDir: string, options: CreateWorkspaceBackupOptions) {
  ...
  try {
    ...
    const summary: WorkspaceBackupSummary = {
      format: "agentbot.workspace-backup", version: 1, id: job.id, createdAt: new Date().toISOString(),
...
function validateManifest(value: unknown): Manifest {
  if (!record(value) || !record(value.summary) || value.summary.format !== "agentbot.workspace-backup" || value.summary.version !== 1 ||
...
async function inspectTar(path: string): Promise<Map<string, { type: string; size: number }>> {
  ...
  // parser can inflate an authenticated but malicious decompression bagentbot.
...
export async function stageWorkspaceBackup(dataDir: string, archivePath: string, options: { password: string; currentAppVersion?: string }) {
  ...
  try {
    ...
    if (versions[0] && versions[1]) {
      for (let i = 0; i < 3; i++) {
        if (versions[0][i] > versions[1][i]) throw new Error("This backup was made by a newer Agentbot version. Update the app before restoring it.");
...
function finishRestore(dataDir: string, id: string, consumePending = true): LastWorkspaceRestore {
  ...
  const draftAttachments = clientState["agentbot-draft-attachments"];
  if (draftAttachments) {
    try {
      ...
      clientState["agentbot-draft-attachments"] = JSON.stringify(value);
