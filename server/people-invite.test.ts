 ...
 * owner, the way `agentbot` on the server or a bootstrap script talks. */
...
beforeAll(async () => {
  ...
  home = mkdtempSync(join(tmpdir(), "agentbot-people-invite-"));
  ...
  writeFileSync(join(home, ".agentbot", "config.json"), JSON.stringify({ instances: { fixture: { driver: "people-invite-test-shadow" } } }));
  child = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
    ...
    env: {
      ...
      AGENTBOT_ENVIRONMENT_LABEL: "acme",
...
describe("adding people to a hosted workspace", () => {
  ...
  it("offers no email sign-in until the owner names the first admin", async () => {
    expect((await remote("/.well-known/agentbot/environment")).body.capabilities.emailSignIn).toBe(false);
    ...
    expect((await remote("/.well-known/agentbot/environment")).body.capabilities.emailSignIn).toBe(true);
