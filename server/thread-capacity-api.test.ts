...
describe("per-bot thread capacity through an isolated HTTP fixture", () => {
  ...
  const capabilityStatus = async (launched: any) => {
    const response = await fetch(`${fixture.info.url}/api/internal/agents`, {
      headers: { authorization: `Bearer ${launched.mcpConfig.mcpServers.agents.env.AGENTBOT_COMMS_TOKEN}` },
    });
  ...
  beforeEach(async () => {
    ...
    writeFileSync(wrapper, [
      ...
      'process.env.AGENTBOT_FIXTURE_LAUNCHED_AT = String(Date.now());',
  ...
  it("defaults to three, runs ten real turns after raising the limit, and safely queues and cancels overflow", async () => {
    ...
    expect(new Set(launched.map((entry) => entry.env.AGENTBOT_FIXTURE_CWD)).size).toBe(10);
    ...
    evidence.push({ simultaneousProviderProcesses: launched.map((entry) => ({ pid: entry.pid, cwd: entry.env.AGENTBOT_FIXTURE_CWD })), model });
  ...
  it("counts approval waits, drains FIFO when raised or a turn finishes, and preserves active work when lowered", async () => {
    ...
    expect(Number(launchedSecond.env.AGENTBOT_FIXTURE_LAUNCHED_AT)).toBeLessThan(Number(launchedThird.env.AGENTBOT_FIXTURE_LAUNCHED_AT));
    expect(Number(launchedThird.env.AGENTBOT_FIXTURE_LAUNCHED_AT)).toBeLessThan(Number(launchedFourth.env.AGENTBOT_FIXTURE_LAUNCHED_AT));
    finish(fourth);
    await expect.poll(() => busyThreads(botId)).toEqual([]);
    for (const [index, threadId] of [second, third, fourth].entries()) {
