...
export function gatedLocalComputer(
  ...
  return {
    ...
    env: {
      ...
      AGENTBOT_CUA_COMMAND: connection.command,
      AGENTBOT_CUA_ARGS: JSON.stringify(connection.args),
      AGENTBOT_CONTROL_URL: control.url,
      AGENTBOT_CONTROL_TOKEN: control.token,
    },
  };
}
...
export function decodeLinuxDescriptor(value: LinuxConnectionDescriptor): LocalComputerConnection | null {
  ...
  if (
    ...
    (mcp.env as Record<string, unknown>).CUA_DRIVER_HOST_BUNDLE_ID !== "com.agentbot.app" ||
...
export function readCuaConnection({
  ...
  userData = process.env.AGENTBOT_USER_DATA,
  home = homedir(),
  ...
  if (platform === "darwin") {
    ...
    for (const directory of ["Agentbot", "agentbot", "OpenGrokBot", "opengrokbot"]) {
