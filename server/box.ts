...
const BOX_API = process.env.AGENTBOT_BOX_API || "https://ascii.dev/api/box/v1";
...
// into every new name so another Agentbot installation using the same Box
 ...
 * carrying Agentbot's exact deterministic name shape leave this boundary;
...
export async function listManagedBoxes(
  ...
  if (options.adoptLegacy !== false) {
    ...
    try {
      ...
      return invalidInventory("Agentbot could not safely read its cloud computer recovery records");
    }
    for (const recovery of recoveries) {
      ...
      if (matchingRows.length > 1) {
        return invalidInventory("ascii.dev returned a conflicting id for an Agentbot-managed cloud computer — refresh or repair it in ascii.dev");
      }
      if (matchingRows.length === 1) {
        ...
        if (listedName !== namedOwner.currentName && listedName !== namedOwner.legacyName) {
          return invalidInventory("A remembered cloud computer no longer has its Agentbot owner name — repair it in ascii.dev before continuing");
        }
        continue;
      }
    ...
    try {
      ...
      return invalidInventory("Agentbot could not safely read its cloud computer deletion records");
    }
  ...
  for (const candidate of candidates) {
    ...
    if (!BOX_ID.test(boxId)) {
      return invalidInventory("ascii.dev returned an invalid id for an Agentbot-managed cloud computer — refresh or repair it in ascii.dev");
    }
    const existing = ownedBoxByBot.get(owner.botId);
    if (existing && existing !== boxId) {
      return invalidInventory("ascii.dev returned conflicting cloud computers for one Agentbot bot — repair them in ascii.dev before continuing");
    }
  ...
  for (const candidate of candidates) {
    ...
    if (legacyOwner && owner && options.adoptLegacy !== false) {
      try {
        ...
        return invalidInventory("Agentbot could not safely remember this legacy cloud computer's owner — repair it in ascii.dev before continuing");
      }
    }
...
async function revalidateManagedBox(
  ...
  if (!instance) {
    throw Object.assign(new Error("that Agentbot-managed cloud computer no longer exists"), { status: 404 });
  }
  return instance;
}
...
/** The keys this Agentbot already holds, as the environment its bots'
...
async function requestBoxCreate(cfg: AppConfig, botId: string, ttlSeconds: number, env: Record<string, string>): Promise<BoxCreateResult> {
  ...
  // guest ever has are the ones this Agentbot forwards (`env`), which its
...
export async function provisionBox(cfg: AppConfig, botId: string, _botName: string) {
  ...
  cfg = snapshotBoxConfig(cfg);
  if (!boxConfigured(cfg)) {
    throw new Error('box provider not enabled — add {"box":{"token":"…"}} to ~/.agentbot/config.json');
  }
...
// Agentbot has over any frame off this box: the turn runs on the box's
