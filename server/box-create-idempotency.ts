...
export interface BoxCreateRequest {
  ...
  createdAt: number;
  boxId?: string;
  /** The provider Box has its deterministic Agentbot name. Until this is
...
function recoveryStateError(detail: string, cause?: unknown): Error & { status: number } {
  return Object.assign(
    new Error(
      ...
      + "Check ascii.dev for an unnamed Box before repairing Agentbot's local state.",
    ),
    { status: 503, cause },
  );
}
  ...
  try {
    for (;;) {
      ...
      if (performance.now() >= deadline) {
        throw recoveryStateError("locked by another Agentbot process");
      }
      if (reaped) continue;
