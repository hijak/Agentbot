// `agentbot service install`: keep the server running across reboots.
// Renders a systemd unit (Linux) or a launchd agent (macOS) that runs the
// same `agentbot serve …` the operator just used, and either installs it
...
export const SYSTEMD_UNIT_NAME = "agentbot.service";
export const LAUNCHD_LABEL = "com.agentbot.serve";
...
export function systemdUnit(spec: ServiceSpec): string {
  const lines = [
    "# Written by `agentbot service install`. Re-run it to change the options.",
    "[Unit]",
    `Description=Agentbot${spec.label ? ` (${spec.label})` : ""}`,
    ...
    `Environment=AGENTBOT_DATA_DIR=${spec.dataDir}`,
...
export function unstableInstallWarning(script: string): string | null {
  ...
  if (/\/_npx\//.test(normalized) || /\/\.npm\/_npx\//.test(normalized)) {
    return `this command runs from an npx cache (${dirname(script)}), which npm may delete at any time. Install it permanently first (npm install -g agentbot) and run \`agentbot service install\` from that install.`;
  }
  return null;
}
...
export function currentUser(): string {
  try {
    ...
    return process.env.USER || process.env.USERNAME || "agentbot";
  }
}
