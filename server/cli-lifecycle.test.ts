...
beforeEach(() => {
  ...
  options = { command: "serve", port: 18451, dataDir, tailscale: false, client: false, pair: false, json: false, guided: true, open: false };
...
describe("CLI startup lifecycle", () => {
  ...
  it("stops the server after SIGINT during the final readiness response", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request) => {
      ...
      return Response.json({ app: "agentbot", pid: childPid });
    }));
    expect(await runServe({ ...options }, vi.fn())).toBe(0);
  ...
  it("reopens a matching desktop workspace even when its server PID differs from the lease owner", async () => {
    ...
    writeFileSync(join(dataDir, "agentbot-server.lease"), JSON.stringify({
      version: 1, pid: process.pid, host: hostname(), token: workspaceId, createdAt: Date.now(),
    }));
    vi.stubEnv("AGENTBOT_DATA_DIR", process.env.AGENTBOT_DATA_DIR);
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request) => String(url).endsWith("/api/health")
      ? Response.json({ app: "agentbot", pid: childPid })
  ...
  it("does not mistake another workspace on the same port for this one", async () => {
    writeFileSync(join(dataDir, "environment-id"), workspaceId);
    vi.stubGlobal("fetch", vi.fn().mockResolvedValueOnce(Response.json({ app: "agentbot", pid: childPid }))
  ...
  it.each(["mismatched", "unreachable"])("never mints a phone code for a %s public endpoint", async (failure) => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (address.endsWith("/api/health")) {
        return ++healthRequests === 1 ? Response.json({}, { status: 503 }) : Response.json({ app: "agentbot", pid: childPid });
      }
    ...
    expect(requests).toContain("https://fixture.example.test/.well-known/agentbot/environment");
  });
  ...
  it("offers Android client pairing only after verifying the same workspace, without claiming it is connected", async () => {
    ...
    // the agentbot:// one (android/core Connection.kt).
    const credential = `omb_pair_${"a".repeat(43)}`;
    const inviteUrl = `agentbot://pair?address=${encodeURIComponent(origin)}&token=${credential}&name=fixture`;
    let healthRequests = 0;
    const fetcher = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (address === `http://127.0.0.1:${options.port}/api/auth/pairing`) {
        ...
        // A server started without AGENTBOT_PUBLIC_URL: it mints the credential but
        ...
        return Response.json({ code, url: null, expiresAt, credential, serverName: "fixture", hint: "set AGENTBOT_PUBLIC_URL" });
      }
      expect(address).toMatch(/\/\.well-known\/agentbot\/environment$/);
      expect(init?.method).not.toBe("POST");
    ...
    expect(requests).toContain(`http://127.0.0.1:${options.port}/.well-known/agentbot/environment`);
    expect(requests).toContain(`${origin}/.well-known/agentbot/environment`);
    expect(requests.indexOf(`${origin}/.well-known/agentbot/environment`)).toBeLessThan(requests.indexOf(pairingRequest));
    ...
    expect(output).toContain("On Android, open the Agentbot app and scan the QR with its pairing scanner.");
