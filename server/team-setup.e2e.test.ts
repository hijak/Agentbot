...
it("Clive reviews multi-provider teams once, continues after each decision, and preserves existing threads through setup and deletion", async () => {
  const gates = mkdtempSync(join(tmpdir(), "agentbot-team-setup-gates-"));
  ...
  try {
    ...
    const start = async (text: string) => {
      ...
      return dump.mcpConfig.mcpServers.agents.env.AGENTBOT_COMMS_TOKEN as string;
    };
    const finish = async () => {
    ...
    ] } }, 201, groupDump.mcpConfig.mcpServers.agents.env.AGENTBOT_COMMS_TOKEN);
