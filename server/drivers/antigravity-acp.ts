...
export async function prepareAntigravityProfile(input: {
  ...
  if (/\r|\n|\0|%s/u.test(executable) || executable.includes(delimiter)) {
    throw new Error("The Agentbot runtime path cannot safely suppress Antigravity browser launches.");
  }
...
export class AntigravityAcpClient {
      ...
      const timer = setTimeout(() => {
        ...
        if (method === "initialize") {
          ...
          reject(new Error(
            ...
            "Retry setup. If it still fails, share this error and your Agentbot version; do not paste Google sign-in links or tokens.",
          ));
  ...
  async initialize(timeoutMs = STARTUP_TIMEOUT_MS): Promise<any> {
    const initialized = await this.request("initialize", {
      protocolVersion: 1,
      clientInfo: { name: "agentbot", version: "0.0.0" },
...
export async function validateAntigravityRuntime(runtime: AntigravityRuntime, expectedVersion: string): Promise<void> {
  const profileDirectory = await mkdtemp(join(tmpdir(), "agentbot-antigravity-verify-"));
