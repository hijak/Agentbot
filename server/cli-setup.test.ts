...
describe("API onboarding", () => {
  it.each<{
  ...
  }>([
    {
      ...
      environment: { AGENTBOT_SETUP_TEST_CUSTOM_KEY: "instance-custom-key", OPENAI_COMPAT_API_KEY: "instance-default-key" },
      processEnvironment: { AGENTBOT_SETUP_TEST_CUSTOM_KEY: "process-custom-key", OPENAI_COMPAT_API_KEY: "process-default-key" },
      expected: "config-key",
    },
    {
    ...
    },
    {
      ...
      processEnvironment: { AGENTBOT_SETUP_TEST_CUSTOM_KEY: "process-custom-key" },
      expected: "instance-default-key",
    },
    {
      label: "custom process environment fallback",
      processEnvironment: { AGENTBOT_SETUP_TEST_CUSTOM_KEY: "process-custom-key" }, expected: "process-custom-key",
    },
    {
    ...
    const original = {
      ...
      config: { url: "https://api.example.test/v1", apiKeyEnv: "AGENTBOT_SETUP_TEST_CUSTOM_KEY", ...(configKey ? { key: configKey } : {}) },
    };
...
describe("CLI startup preferences", () => {
  ...
  it("refuses to save while the server owns the data directory", () => {
    ...
    const lease = acquireDataDirLease(DATA_DIR);
    try {
      expect(() => saveCliStartup(DATA_DIR, { access: "tailscale", phone: "android" })).toThrow(/already using this data directory/);
      expect(readFileSync(configPath, "utf8")).toBe(original);
