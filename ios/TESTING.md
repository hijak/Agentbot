...
```sh
git clone https://github.com/milind-soni/Agentbot
cd Agentbot
...
dns-sd -B _agentbot._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/Agentbot must be allowed, or the phone reaches
...
cd ios && xcodegen generate && open AgentbotCompanion.xcodeproj
```
...
```sh
xcodebuild -project AgentbotCompanion.xcodeproj -scheme AgentbotCompanion \
...
1. **Pair.** In Agentbot → Settings → Phone, choose **Pair a phone**.
   Scan the QR code with the phone's Camera, open AgentbotMobile,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → Agentbot → Local Network. If the
...
3. **In Agentbot → Settings → Phone:** with Phone access on, the panel now
...
- **Xcode console**, subsystem `com.agentbot.companion`: `opening stream`,
