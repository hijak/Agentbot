...
enum AgentbotSharedKeychain {
    private static let service = "com.agentbot.companion.token"
    ...
    static func remove(_ connectionID: String) -> Bool {
        guard let accessGroup = AgentbotSharedConfiguration.keychainAccessGroup else {
            return false
        }
    ...
    static func removeIncludingLegacyItem(_ connectionID: String) -> Bool {
        ...
        guard let legacyGroup = AgentbotSharedConfiguration.legacyAppKeychainAccessGroup else {
            return false
        }
    ...
    private static func requiredAccessGroup() throws -> String {
        guard let accessGroup = AgentbotSharedConfiguration.keychainAccessGroup else {
            throw AgentbotSharedKeychainError.configurationMissing
        }
        return accessGroup
    }

    private static func requiredLegacyAccessGroup() throws -> String {
        guard let accessGroup = AgentbotSharedConfiguration.legacyAppKeychainAccessGroup else {
            throw AgentbotSharedKeychainError.configurationMissing
        }
        return accessGroup
    }
    ...
    private static func save(
        ...
        guard status == errSecSuccess else {
            throw AgentbotSharedKeychainError.security(status)
        }
    }
    ...
    private static func readToken(
        ...
        guard let data = item as? Data,
            ...
            throw AgentbotSharedKeychainError.security(errSecDecode)
        }
        return token
    }
...
enum AgentbotSharedKeychainError: LocalizedError {
