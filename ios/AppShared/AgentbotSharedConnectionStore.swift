...
enum AgentbotConnectionRegistrySource: Equatable {
    case sharedRegistry
...
struct AgentbotConnectionRegistryResolution: Equatable {
    let registry: CompanionConnectionRegistry
    let source: AgentbotConnectionRegistrySource
}

/// Non-secret pairing metadata shared with extensions.
///
/// Tokens never enter defaults; they live in `AgentbotSharedKeychain`. The
...
enum AgentbotSharedConnectionStore {
    ...
    static func resolve(
    ...
    ) -> AgentbotConnectionRegistryResolution {
        ...
        if let sharedRegistryData,
           let registry = try? decoder.decode(
            ...
            return AgentbotConnectionRegistryResolution(
                registry: registry,
    ...
    static func loadRegistry(
        sharedDefaults: UserDefaults? = AgentbotSharedConfiguration.sharedDefaults,
