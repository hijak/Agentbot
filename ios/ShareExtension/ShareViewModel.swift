...
final class ShareViewModel: ObservableObject {
    ...
    private func performRetry() async {
        ...
        if let pendingDelivery {
            ...
            phase = .loading
            do {
                let registry = AgentbotSharedConnectionStore.loadRegistry()
    ...
    func chooseComputer(_ id: String) async {
        guard id != selectedComputerID,
              ...
              let selected = AgentbotSharedConnectionStore.loadRegistry().connection(id: id)
        else { return }
    ...
    private func deliver(
        ...
        do {
            ...
            try Task.checkCancellation()
            if let connection {
                AgentbotSharedConfiguration.sharedDefaults?.set(
    ...
    private func connect(to selectedConnection: Connection) async throws {
        guard let pairedToken = try AgentbotSharedKeychain.token(for: selectedConnection.id) else {
            throw ShareExtensionError.notPaired
        }
        ...
        let remembered = AgentbotSharedConfiguration.sharedDefaults?
    ...
    private func friendlyMessage(for error: Error) -> String {
        if let apiError = error as? APIError {
            if apiError.isUnauthorized {
                return "This phone's pairing has expired. Open Agentbot and pair it again."
            }
            if isAmbiguousTransport(error) {
                return "Couldn't reach your computer. Keep Agentbot open and Phone access on, then try again."
            }
        }
        return (error as? LocalizedError)?.errorDescription
            ?? "Agentbot couldn't send this. Please try again."
    }
...
private enum ShareExtensionError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notPaired:
            return "Open the Agentbot app once after updating. If this phone still isn't connected, pair it before sharing."
        case .noDestinations:
            return "There aren't any bots or channels to send this to yet. Create one on your computer first."
        case .imageSupportUnavailable:
            return "Update Agentbot on this computer before sharing images."
        case let .offline(name):
            return "Couldn't reach \(name). Keep Agentbot open and Phone access on, then try again."
        case .sendTimedOut:
