...
enum ShareItemLoadingError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .appGroupUnavailable:
            return "Agentbot couldn't open its secure sharing folder. Open the app once, then try again."
        case .nothingSupported:
        ...
        case let .unreadable(name):
            return "Agentbot couldn't read \(name). Try exporting it to Files first."
        }
    }
}
...
enum ShareItemLoader {
    ...
    static func load(from context: NSExtensionContext) async throws -> LoadedShareItems {
        ...
        AgentbotSharedInbox.removeDirectories(olderThan: 0)
        guard let container = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: AgentbotSharedConfiguration.appGroupIdentifier
        ) else { throw ShareItemLoadingError.appGroupUnavailable }

        let inboxRoot = container.appendingPathComponent(
            AgentbotSharedInbox.directoryName,
            isDirectory: true
        )
        let inbox = inboxRoot
