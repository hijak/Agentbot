...
enum Keychain {
    static func save(_ token: String, for connectionId: String) throws {
        try AgentbotSharedKeychain.save(token, for: connectionId)
    }
    ...
    static func token(for connectionId: String) throws -> String? {
        try AgentbotSharedKeychain.tokenMigratingLegacyItem(for: connectionId)
    }

    @discardableResult
    static func remove(_ connectionId: String) -> Bool {
        AgentbotSharedKeychain.removeIncludingLegacyItem(connectionId)
    }
}

typealias KeychainError = AgentbotSharedKeychainError
