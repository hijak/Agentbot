...
`pnpm dev:desktop` launches the Electron shell (macOS/Ubuntu; keep server + Vite running).
...
- stable `Agentbot-amd64.deb` and `Agentbot.AppImage` copies used by the latest-download links;
...
[Agentbot releases](https://github.com/milind-soni/Agentbot/releases). Then verify the checksum
...
Data lives in `~/.agentbot/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.agentbot`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.agentbot/config.json` via `PUT /api/config` and the API
...
Agentbot release repository, and never change the upstream publish target in a feature PR unless
